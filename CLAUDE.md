...
- **(A) ダウンロードの成否を確認する手段が無い**: ブラウザは1つのページが
  ...
  **破棄されても例外を投げない**ため、アプリからは成功と区別が付かず、
  ...
  まとめて無効化し、加えてモジュール変数`apkgExportInProgress`でも弾く
  (連打・無効化前の実行に備えた二重の防御)。**呼び出し側は`beginApkgExport`が
  trueを返したら必ず`finally`で`endApkgExport()`を呼ぶこと**(でないと以後
  どのタブからも出力できなくなる)。
...
テストは`test_web_ui.mjs`の[27]。**無効化されたボタンは`click()`しても
ハンドラ自体が呼ばれない**ため、(a)ボタンが無効になること、(b)無効なボタンを
押しても何も起きないこと、(c)`disabled`を手で外して押した場合はコード側が
弾くこと、の3段で確認している(最初この違いを踏まえずに書いてテストが失敗した)。
