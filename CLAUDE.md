...
- **入力表現は例文に流用しない**。文の経路(`generateShuujukuItemsFromSentences`)
  ...
  例文にできない。代わりに**その表現を含む例文を3つ**生成させる。
...
- **入力件数に応じて「組み合わせ例文」を増やす**(2026-08-06、片桐の選択)。
  ...
  - **組み合わせ数は「チャンクの件数」ではなく「入力全体の件数」から決める**。
    10件を超えて`BATCH_SIZE`ごとに分かれた場合、チャンクの件数で決めると
    最後の1件だけ例文数が減るという分かりにくい差が出る。組み合わせ相手の
    一覧(`{{all_phrases}}`)も入力全体を毎チャンクに渡している。
  - プロンプトには「組み合わせのために不自然な文を作らないこと。自然に
    繋がらない場合はその表現だけの例文で代えてよい」と逃げ道を書いてある
    (無関係な語句同士を無理に1文へ詰め込ませないため)。
  - テスト(`test_web_ui.mjs`の[29])は**送信したプロンプトの中身**を検証する。
