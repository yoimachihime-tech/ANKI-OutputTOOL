...
- **【提案済み・片桐の判断待ち】2026-08-05のバグ検査で挙がった改善案の残り**:
  ...
  併せて提案した改善案6件のうち、**1・2・6は同日実装済み**
  (下記「起動時の自動読み込み」「バックアップ(書き出し/読み込み)」、および
  `_old_anki_tts_gui.py`の削除)。**まだ着手していない**のは次の3件。
  - **tombstoneの刈り込み** — 無期限に増え続け、同じ50,000文字セルを
    itemsと食い合う。件数か日数で上限を設けるだけで済む。ただし刈り込み
    すぎると「削除済み項目が復活する」ため閾値の設計は慎重に。
    2026-08-05に上限の事前ガードを入れたので静かに壊れることはなくなり、
    緊急性は下がっている。
  - **GitHub ActionsでのCI** — `docs/`はテスト7本が揃っているので
    `npm test`を回すだけで回帰を自動化できる(公開リポジトリなので無料枠)。
    **注意**: `verify`系2本が`python3`をハードコードしているため、
    ワークフロー側にPythonのセットアップが要る。
