...
`npm test`(6本、169アサーション)は全て通過している。**片桐の実機確認は
ログイン(GISテストユーザー追加後)・シート読み込みまで完了済み**
(2026-07-29)。英文添削→シート追記・apkg出力→Anki出力済みマークの
実機確認はまだ)。一覧のスクロール化・重複/誤りなしフィルターも同日
追加済み(下記「実装上の注意」参照)。
...
- **実機確認(2026-07-29)でログイン・シート読み込みまで動作することを
  ...
  - **選択のID化**: フィルターで表示行が絞られると描画順序と
    ...
    `onDailyExcludeSelected()`を変更した(位置ベースの
    `checkedIndicesOf`は単語/AIに質問/習熟用タブで従来通り使用)。
