...
- **【提案済み・片桐の判断待ち】2026-08-05のバグ検査で挙がった改善案の残り**:
  ...
  `_old_anki_tts_gui.py`の削除)。**まだ着手していない**のは次の1件だけ。
  - **デスクトップ版とWeb版のストック統合** — 今は`*_stock.json`と
    localStorage/`_AppSync`が完全に分断されており、デスクトップで作った
    候補はWebから見えない(逆も同様)。デスクトップ側からも`_AppSync`を
