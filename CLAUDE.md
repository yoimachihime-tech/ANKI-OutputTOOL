...
- **【提案済み・片桐の判断待ち】2026-08-05のバグ検査で挙がった改善案**:
  同日の検査で見つかったバグ8件は修正済み(下記「2026-08-05のバグ修正」)。
  併せて提案し、**まだ着手していない**改善案は次のとおり(優先度順)。
  1. **同期の自動化** — 現状は手動ボタンのみで、押し忘れが衝突の主因になる。
     まず「起動時に自動pull」だけでも入れると、端末を持ち替えた直後の
     事故が大きく減る。
  2. **ストックのJSONエクスポート/インポート** — Web版に自動バックアップが
     無いのは明確な弱点(下記「出力済みカードの削除」の項を参照)。
     File System Access APIは不要で`downloadBlob`の再利用だけで実現でき、
     「出力済みを削除」の直前に促す導線にすればセル容量問題とも噛み合う。
  3. **tombstoneの刈り込み** — 無期限に増え続け、同じ50,000文字セルを
     itemsと食い合う。件数か日数で上限を設けるだけで済む。
  4. **GitHub ActionsでのCI** — `docs/`はテスト7本が揃っているので
     `npm test`を回すだけで回帰を自動化できる(公開リポジトリなので無料枠)。
  5. **デスクトップ版とWeb版のストック統合** — 今は`*_stock.json`と
     localStorage/`_AppSync`が完全に分断されている。認証方式が違う
     (サービスアカウント vs OAuth)ため設計検討が必要。中〜大規模。
  6. **`_old_anki_tts_gui.py`の削除** — 参照用の旧版(56KB)が残っており、
     編集対象の誤認を招く。
