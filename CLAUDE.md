...
すべて混在していたが、責務ごとに分割済み。**分割後のファイル(`tts_gui.py` /
`tts_core.py` ほか)が正典**。参照用に残していた旧版 `_old_anki_tts_gui.py` は、
編集対象の誤認を招くため2026-08-05に削除した(必要になった場合は
`git show 25dc499:_old_anki_tts_gui.py` でgit履歴から取り出せる)。
...
tools/repair_sync_usn.py
                  「同期のたびに大量ダウンロードが発生する」状態(col.usnと
                  行のusnの不整合)を直すスクリプト(2026-08-20追加。上記の
                  移行スクリプトの初版が作ってしまった不具合の復旧用)
...
tab_notes_state.py
                  ...
                  下記「ノート一覧の永続化」参照)
json_store.py     上記の永続化JSON(ストック・カード定義・タブ状態・除外リスト)を
                  安全に読み書きする共通モジュール(2026-08-05追加、下記
                  「永続化JSONの安全な読み書き」参照)
test_json_store.py
...
local_tts_server.py
                  「既存のapkgにTTS音声を付ける」だけを行うローカルサーバー
                  (2026-08-19追加。画面もこのサーバーが配信する。下記
                  「ローカルのTTSツール」を参照)
local_tts_page.html
                  上記が配信する画面(1枚のHTML。外部依存なし)
apkgにTTS音声を付ける.bat
                  local_tts_server.py の起動用バッチ(ブラウザも自動で開く)
tools/localhost_cors_check.py
                  「GitHub Pagesのページからローカルへfetchできるか」を確かめる
                  ための使い捨ての疎通確認サーバー(2026-08-19追加。結論は
                  「できない」。下記「ローカルのTTSツール」を参照)
tools/register_local_tts_protocol.py
                  Web版の「🔊 PCのTTSツールを起動する」ボタンを効かせるための
                  カスタムURLスキーム(ankitts:)登録スクリプト(2026-08-19追加。
                  PCごとに1回だけ実行する。HKCUのみ・管理者権限不要)
```

### 永続化JSONの安全な読み書き(json_store.py、2026-08-05追加)

`word_stock.py` / `shuujuku_stock.py` / `grammar_multi_stock.py` /
`card_defs.py` / `tab_notes_state.py` / `daily_pending_exclusions.py` の6つは、
いずれも自分専用のJSONファイルを持つが、**読み書きの実体は`json_store.py`に
集約してある**。新しく永続化ファイルを増やす場合も、直接`open()`せず
`json_store.read_json()` / `json_store.write_json()` を使うこと。

- **なぜ集約したか**: それまで6モジュールとも
  `with open(path, "w") as f: json.dump(...)` という同じ書き方をしていた。
  `open(path, "w")`は**開いた時点で既存の中身を切り捨てる**ため、書き込み中に
  プロセスが落ちる・Google Driveの同期がファイルをロックすると、中途半端な
  JSONだけが残る。さらに読み込み側の`json.load()`にも例外処理が無かったため、
  一度壊れると次回以降`JSONDecodeError`で起動時から操作不能になり、
  **片桐の実データ(未出力の単語・習熟用候補など)が失われる**構成だった。
  このフォルダがGoogle Drive同期下にあること、テストで実データを壊す事故を
  過去に2回起こしていることを踏まえた対応。
- `write_json(path, data)`: 同じフォルダの一時ファイルへ書き切ってから
  `os.replace()`で置き換える。`os.replace()`はWindowsでも上書きが
  アトミックであることが保証されているため、途中で落ちても**旧版が丸ごと
  残る**。一時ファイルを**同じフォルダに**作るのは、別ボリュームだと
  `os.replace()`がアトミックにならないため。`fsync()`まで行う。
- `read_json(path, default)`: 壊れていた場合は例外を投げず、そのファイルを
  `<path>.corrupt`へ退避してから`default`を返す。黙って空にせず退避するのは、
  末尾が切れただけなら手作業で中身を救出できる余地を残すため。
- テストは`test_json_store.py`(`python test_json_store.py`、17件)。
  **各モジュールの`STOCK_PATH`等を一時フォルダへ差し替えてから実行する**
  (実データに触れないため。この差し替えは必ず最初に行うこと)。
...
- **見出しラベルの結合(2026-07-27) → 除去(2026-08-21)**: 「Ex1. ...」
  「2. ...」のような見出しラベル付きの文(AIに質問/DailyConversationの
  Example、単語タブのExample等で使われる形式)は、単純な句点区切りだと
  `"Ex1."`単体が1つの「文」として切り出されてしまい、TTS生成時にラベルだけの
  極小mp3が大量発生してAnkiコレクションを圧迫していた。2026-07-27はこれを
  「次の断片(実際の文)へ**結合**する」ことで回避していたが、**結合ではラベル
  ごと読み上げられる**という別の問題が残っていた。2026-08-21に**除去**へ
  変更した(下記)。`_LABEL_ONLY_RE`(英字0〜6文字+数字1〜3文字+句点)は
  少なくとも1桁の数字を要求しているため、"Yes." "No."のような正当な短文を
  ラベルと誤認することはない。
- **「表示のためだけの文字」を読み上げから外す(2026-08-21追加、A案)**:
  片桐から**「AIに質問タブで生成したカードにTTS音声を付けると、『ex2』などの
  近辺で描画されていない文字の読み上げが入ってしまう」**と報告を受けての修正。

  原因は、カードの見た目のためにフィールドへ焼き込まれている文字が、そのまま
  TTSのソーステキストになっていたこと。`strip_html_for_tts`は`<[^>]+>`で
  タグを消すが、**`<span class="ex-num">Ex1.</span>`の`<span>`が消えても中身の
  「Ex1.」は残る**。片桐のコレクション内の生成済みmp3を実測すると:
  ...
  語数が少ないほうが倍近く長く、差の約1.9秒がラベルの読み上げ("E X one"+
  文末ポーズ)だった。Grammar Multiの155ノート全件にラベルが入っており、
  うち82件はEx2以降まである。Answerも88件が「(B) 」で始まり、"B"が
  読まれていた。

  対策として`strip_display_only_markup()`を追加し、**タグ除去より前に**
  次の3つを落とすようにした:

  - 例文の採番ラベル`<span class="ex-num">Ex1.</span>`(タグごと中身を削除)
  - Answer先頭の選択肢記号「(B) 」(選択肢は(A)〜(D)しか使わないので、
    "(I) am ..." のような正当な文を巻き込まないよう範囲を絞ってある)
  - spanで囲まれていない素の見出しラベル(`_LEADING_LABEL_RE`)

  あわせて`<br>`・`</div>`の扱いも直した。以前は無条件に`". "`へ置換して
  いたため「…nothing.<br>…」が「…nothing.. Ex2.…」と句点が二重になり、
  **Ex2の直前だけ余分な間が空いていた**。直前が「.」「!」「?」で
  終わっている場合は句点を足さない。

  実装上の要点として、`strip_html_for_tts`と`split_into_sentences`の実体を
  `_tts_sentences_from_html()`1つに統合した。**片方だけ直すと、実際の
  読み上げ内容と`analyze_targets`の文字数見積もりがずれる**ため
  (`local_tts_server._tts_text`のコメントも同じ注意を書いている)。

  Web版`docs/lib/tts.js`にも同じ`stripDisplayOnlyMarkup()`/
  `ttsSentencesFromHtml()`を入れてある。回帰テストは
  **`test_tts_core.py`(Python)と`tools/test_tts.mjs`の[1]/[1b](Web版)**に
  同じ固定ケースで置いた。どちらかを変えたら必ず両方を更新すること。

  **なお、これはフィールドの中身を書き換える修正ではない**(読み上げに渡す
  テキストだけを変える)。カード上の「Ex1.」「Ex2.」の表示はこれまでどおり
  残る。既存ノートの音声を作り直したい場合は、④で「音声を作り直す
  (force_regen)」を使う。
...
- 「① 入力元を選択」セクション(画面最上部): タブ切替式、**5タブ**。入力源が
  ...
  - **単語タブ**(実装済み、2026-07-27追加): 下記「単語カード生成との関係」を
    ...
    **`word_stock.py`(word_stock.json)という完全に別のストックファイルを使い、
    ...
    としないようにしてある)。`on_word_generate_clicked`が全ペアをまとめて
    `gemini_client.generate_vocab_cards_from_words`に渡し(2026-08-06に
    1件ずつ直列で呼ぶ方式から変更。下記「まとめて生成(バッチ)」参照)、生成できたものを
...
- `generate_shuujuku_items_from_rows(rows, api_key, model)`: DailyConversationの
  シート行から、文法パターンの抽象化・新規例文の創作をGeminiにやらせ、
  `build_shuujuku_v1.build_deck()`向けのitem dictを**まとめて**返す
  (2026-08-06にバッチ化。戻り値は`rows`と同じ長さで、生成できなかった件は
  `None`。下記「まとめて生成(バッチ)」参照)。
...
#### 【重要】colテーブルのusnは絶対に触らないこと(2026-08-20の不具合)

移行スクリプトの初版が、スキーマ変更を知らせるつもりで
`update col set scm=?, mod=?, usn=-1` と**colのusnまで-1にしてしまった**。

- notes/cards/revlogの`usn = -1`は「ローカルで変更した、未アップロード」の印。
- **colテーブルのusnは意味がまったく違い、「最後に同期が成功した時点の
  サーバ側USN」**を覚えておく値。

ここを-1にすると「この端末はUSN -1の時点までしか同期していない」状態になり、
以後の同期のたびにサーバが「それより新しい行」=コレクションのほぼ全体を
送り直してくる。片桐の環境では `col.usn = 5` に対して usn がそれより大きい行が
**175,166件**(notes 6,153 / cards 8,399 / revlog 160,614)あり、
「同期が毎回数分かかる。直後にもう一度同期すると速いが、しばらく経つとまた
長くなる」という症状になった。移行前のバックアップでは
`col.usn = 15613` / 行のusn最大 = 15611 で、
**`col.usn >= すべての行のusn`** という関係が保たれていた。

復旧は `tools/repair_sync_usn.py`(下見が既定、`--apply`で実行)。行側のusnを
`col.usn`まで下げて上の関係を戻し、`col.scm`を進めて全同期を要求させる。
**サーバ側にも古いusnの行が残っているのでローカルを直すだけでは再発する**。
必ず「アップロード」でサーバを作り直すこと(2026-08-20 16:58の全同期が効か
なかったのはこのため——あのときアップロードしたファイル自体が、
col.usnだけ小さく行のusnは大きいという不整合を抱えたままだった)。
usnは同期の記帳専用の列で、カードの内容・スケジューリング・復習履歴には
一切関係しない(修復スクリプトはusn以外の全列が変わらないことを検証済み)。
...
`gemini_client.generate_vocab_cards_from_words(pairs, api_key, model)`が、
当初の「ANKI Term」プロンプトのスタイルルール(アスタリスク禁止・
強調は`<b></b>`・半角角括弧禁止・言語分離・`<br>`で改行)を踏襲しつつ、
実際の8フィールド構成に合わせてプロンプトを再設計したもの。`word`フィールドは
AIに生成させず、入力された単語をそのまま使う(表記ゆれ防止のため)。
JSON形式(`_extract_json_array`と同じ抽出方式)で、1件につき
index/reading/pos/meaning/example/example_ja/example_blank/noteの8キーを持つ
オブジェクトの配列を受け取る(2026-08-06にバッチ化。下記
「まとめて生成(バッチ)」参照)。
...
Web版にも同じタブがある(下記「Web版の一括出力タブ」を参照)。
...
### Web版の一括出力タブ(2026-08-21追加)

デスクトップ版と同じ考え方でWeb版(`docs/`)にも用意した。違いは実装上の
制約から来る次の2点だけ。

- **DailyConversationはシートを読みに行かない**。デスクトップ版は一括出力の
  中でシートを読むが、Web版はGoogleログイン(OAuth)が絡むため、ネットワーク
  アクセスと再ログインを一括出力の副作用にしたくない。DailyConversationタブの
  ②で読み込み済みの行(`dailyPendingRows`)だけを対象にする。
- **シートの「Anki出力済み」列へのマークも行わない**(ローカルの出力済みIDは
  `dailyconv.addExportedIds()`で記録する)。必要ならDailyConversationタブから。

複数種別を1つのapkgにまとめるのは`docs/lib/apkg.js`の
`buildApkg({groups: [{cardDef, items, startDue}, ...]})`。従来の
単体指定(`cardDef`/`items`/`startDue`)は「1件だけのgroups」として正規化される
ので、既存の呼び出し側と`tools/verify_web_parity.mjs`は無変更のまま通る。
ノートIDは**種別をまたいで通し番号**にすること(種別ごとに振り直すと衝突する)。

回帰テストは`tools/test_web_ui.mjs`の[19](buildApkgを直接呼び、2デッキ・
2ノートタイプ・3カードが1つのapkgに入ることを確認)と[20](タブのUIと
書き出し、出力後にストックと内訳が更新されること)。

### Web版の設定画面の見やすさ(2026-08-21)

デスクトップ版と同じ指摘への対応。Web版は元から`<h3>`+`<hr>`でグループ分け
されていたが、(a)先頭2グループ(Gemini APIキー/モデル、TTS音声)に見出しが
無く、(b)`h3`が本文と同じ大きさ・色で切れ目に見えなかった。

- 不足していた見出しを追加(`Gemini APIキーとモデル` / `TTS音声(Cloud
  Text-to-Speech)`)。
- `#settings h3`をアクセント色+太字にし、上に余白と区切り線を置いた
  (`docs/style.css`)。`#settings .hint`は左に余白を入れて一段下げ、直前の
  項目にぶら下がって見えるようにした。**説明文の中身は減らしていない**。
...
- Anki側のレンダリングエンジンは `[sound:]` タグをCSS/JSより先に処理するため、
  ...
  - **2026-08-21: このルールに違反していた箇所を実際に見つけて直した**。
    Grammar Multiの「4. 例文穴埋め」は、表で `{{Example}}` を `.masked` 付きで
    出し、CSSで `<b>` を透明にして空所に見せていたが、**音声タグも同じ
    Example フィールドに入る**ため、隠した語が音声で丸聞こえだった
    (片桐のコレクションで41ノートが該当。実レンダリングで、表の
    `question_av_tags` に音声が1件、透明化される語が `['were','would',…]` と
    確認)。対策は下の「例文穴埋めの作り直し」を参照。

### 「4. 例文穴埋め」の作り直しと、テンプレート構成の統一(2026-08-21)

片桐から「別件も修正したい」との指示を受けての対応。実データを調べたところ、
このテンプレートは**2つの向きに壊れていた**:

| 経路 | ノート数 | 症状 |
| --- | --- | --- |
| 旧AwesomeTTS/HyperTTS製 | 41 | `<b>` があるので語は透明になるが、同じフィールドの音声が答えを読み上げる |
| 本ツール製 | 90 | Example に `<b>` が**1つも無い**ため穴が開かず、表に完全な例文と音声がそのまま出る |

さらに、テンプレート3・4にあった
`<div id="qb-source" style="display:none">{{Question}}</div>` + JSのヒント抽出は、
目印 `<b>状況:</b>` が**本ツール製ノートに1件も無い**(0/90。旧アドオン製は
62/65)ため常に空振りし、`blank-hint` は常に空だった。Questionフィールド全体を
非表示でDOMに置くのは、②でQuestionを読み上げ対象に選んだときに
「画面に無いのに読まれる」事故のもとでもある。

**採った対策(片桐の判断)**:

1. **`ExampleBlank` フィールドを末尾に追加**(音声タグを持たない穴あき版)。
   「4. 例文穴埋め」の表はこれだけを出し、完全版の `{{Example}}` と音声は
   **裏にだけ**置く。単語ノートタイプが既に採っている `ExampleBlank` 方式と
   同じ考え方で、隠したい答えと音声を物理的に分離した。
   - `req` は `all[ExampleBlank]` になるので、**穴あき版が無いノートでは
     4枚目のカードがそもそも作られない**(穴が開かないのに出題する事故を
     構造的に防ぐ)。
   - 生成は `example_blank()`。Geminiに例文中の学習対象語を `<b>` で囲ませ
     (共有プロンプトの規則7)、`<b>…</b>` を `<span class="blank">____</span>`
     へ置き換える。`<b>` が1つも無ければ**空文字を返す**。
   - **フィールドは必ず末尾に足すこと**。途中に挿入すると既存ノートの
     フィールド順がずれ、中身が別のフィールドへ移動してしまう。
2. **`qb-source` + JS のヒント抽出を撤去**。穴埋めの表は
   `{{ExampleBlank}}` + `{{ExampleJA}}`(日本語訳が事実上のヒントになる)で
   完結するので、専用のヒントフィールドは足していない。
3. **裏は `{{FrontSide}}` を使う**。旧afmtは `{{Example}}` を2箇所に出して
   いたため、**同じ音声が裏で2回鳴っていた**(`answer_av_tags` に同じファイルが
   2件)。`{{FrontSide}}` はAnkiが `[sound:]` を取り除くので二重再生しない。
4. **テンプレート構成を実態(4本)に統一**。このファイルには長らく
   「片桐がGUIでテンプレート2〜4を削除したので1本だけ」と書いてあったが、
   **実際のコレクションには4つとも残っていた**。その食い違いのせいで、
   2026-08-20〜21に作った24ノートは1枚しか生えず、それ以前の131ノートは
   4枚、という不揃いな状態だった(カード2〜4はノート作成の最大22.3日後に
   後から生成されていた)。`build_grammar_multi_v1_updated.py` に4本とも
   定義し直した。

**!! 新しいapkgを取り込む前に、必ず移行を先に済ませること !!**
ツールが出力するノートタイプは9フィールドになった。**移行していない
コレクション(8フィールド)に新しいapkgを取り込むと、Ankiは同じ名前で別IDの
ノートタイプ「Grammar Multi (文法・複数出題形式)+」を作ってしまう**
(2026-08-21に実データのコピーで確認: 元の155ノートは旧ノートタイプに残り、
新しい1ノートだけが「+」側へ入った)。移行を先に済ませてから取り込めば
ノートタイプは1つのまま(同コピーで 155→156 ノート、id は不変を確認済み)。

**既存コレクションの移行**: `tools/migrate_grammar_multi_blank.py`。
`migrate_shuujuku_notetype.py` と同じ方針で、notes と notetype 定義だけを
変更し cards/revlog は直接触らない(ノートID・カードIDが変わらないので
間隔・FSRSの状態・復習履歴はそのまま残る)。既定は下見で、`--apply` で実行。
Anki起動中は何もせず終了し、実行前に backup/ へ自動退避する。
実行後は**同期で必ず「アップロード」を選ぶこと**(フィールド追加=スキーマ変更)。

片桐のコレクションでの下見結果(2026-08-21):

```
ノート数: 155
  ExampleBlank を埋める        : 61 件   (Example に <b> があるもの)
  <b> が無く穴あき版を作れない  : 94 件
  → 空カードになる「4. 例文穴埋め」: 70 枚(失われる復習履歴 0 回)
```

空カードは**放置すると復習時に「The front of this card is blank.」として
出題されてしまう**ので、[ツール]→[空のカードを削除] で消すこと。
上記のとおりこの70枚に復習履歴は付いていないため、削除しても失うものは無い。

**回帰テスト**: `tools/test_web_ui.mjs` の [10] で、テンプレートごとの
カード枚数(`expectedOrdCounts`)を固定した——「穴あき版が無いノートには
穴埋めカードが生えない」ことがこれで守られる(期待値は2026-08-29の
カード2・3の作り直しで `{0:3, 1:1, 2:3, 3:1}` に変わった。下記参照)。
`tools/verify_grammar_multi_parity.mjs` は `example_blank` も照合対象に加え、
固定ケースの例文に `<b>` を入れて実際に穴あき版が作られる経路を通してある。

### カード2・3の作り直し(2026-08-29)

片桐から**「AIに質問タブで生成したカードの2〜3枚目が、答えが最初から出て
いたり選択肢がそもそも出てこなかったりで勉強に支障がある。1問目は問題無い」**
と報告を受けての修正。実データ(170ノート)を調べたところ、**テンプレート2・3が
旧・手書きノート向けの設計のまま**で、ツール製ノートに合っていなかった。

| 経路 | 件数 | Question | Answer |
| --- | --- | --- | --- |
| 旧アドオン/手書き | 65 | 「指示: … 状況: …」の自己完結した長文 | 正解の**全文** + `(A) — ` |
| 本ツール製 | 105 | 「選択肢から選びなさい」= 選択肢前提 | `(A) ` + 文(語句だけのことも) |

**カード2「2. セルフチェック」は、どちらに転んでも壊れていた**:

- 選択肢ありのノート(94件): 表に `Choices` を出さない設計なので、
  「選択肢から選びなさい」と書いてあるのに**選択肢が出ず答えようがない**。
  裏の「Why not the others?」も、一度も見ていない選択肢の話をしていた。
- 選択肢なしのノート(76件): `Choices` が空だと**カード1側の `{{#Choices}}`
  ブロックも消える**ため、カード1と表・裏がまったく同一の重複カードだった。

**カード3「3. 理由想起」は表に答えと"答えの音声"が出ていた**。表が
`{{Answer}}` だけで `Question` を出さないため、
`Answer` が「(A) of」のようなノート(10件)では表が
「選択問題 / (A) of / Why is this correct?」だけになっていた。さらに
**170件すべての `Answer` に `[sound:]` が入っている**(TTS対象が Answer+Example)
ので、**表を開いた瞬間に正解が読み上げられていた**。裏は `{{FrontSide}}` で
Ankiが音声を除去するため、**音声だけが表にある**という逆転が起きていた。

2026-08-21に「テンプレート構成を実態(4本)に統一」した際、**中身がツール製
データに合うかまでは検証しなかった**のが原因。

**対策**(片桐の選択):

1. **`AnswerPlain` フィールドを末尾に追加**(正解ラベル `(A) ` も `[sound:]` も
   持たない正解文)。TTSの既定選択は名前が完全一致する `Answer`/`Example` だけ
   (`_default_tts_fields` / `TTS_FIELD_KEYS.ai_ask`)なので、**音声は付かない**。
   **フィールドは必ず末尾に足すこと**(途中に挿入すると既存ノートの中身が
   別のフィールドへ移動する)。
2. **カード2の qfmt 全体を `{{#Choices}}` で囲む** → `req` が `all[Choices]` に
   なり、選択肢のあるノートにだけ生える(重複が構造的に消える)。見出しも
   `Self-Check (no options)` から**日本語**の「選択肢を見ずに答える」へ
   ——選択肢が出ないのが仕様だと伝わっていなかったため。**裏では選択肢も
   見せる**(でないと「Why not the others?」が意味を持たない)。
3. **カード3の表を `Question` + `AnswerPlain` にする**。`req` は
   `all[AnswerPlain]`。**音声の入る `Answer` は表に出さない**。

これで4枚の役割が重ならなくなる: 1=答えは?(再認) / 2=答えは?(想起、
選択肢ありのみ) / 3=なぜ?(理由) / 4=空所補充。

**既存コレクションの移行**: `tools/migrate_grammar_multi_answerplain.py`
(`migrate_grammar_multi_blank.py` と同じ方針。notes と notetype 定義だけを
変更し cards/revlog は触らない)。既定は下見、`--apply` で実行。
**新しいapkgを取り込む前に必ずこの移行を先に済ませること**(9フィールドの
コレクションに10フィールドのapkgを入れると、Ankiが「Grammar Multi
(文法・複数出題形式)+」という別ノートタイプを作ってしまう)。

片桐のコレクションのコピーで実行して検証済み(2026-08-29):

```
AnswerPlain を埋める : 170件(Answerが空で埋められないものは0件)
既存カード 541枚: 学習状態(ivl/due/queue/type/reps/lapses/factor)の変化 0枚 /
                  削除 0枚 / revlog 536件も不変
新規に生えたカード 20枚: ord2 が15枚(AnswerPlainが入ったノート)、
                        ord1 が5枚(Choicesを持つのに未生成だったノート)
空カードになる「2. セルフチェック」: 66枚(失われる復習履歴 21回)
```

空カードは**放置すると復習時に「The front of this card is blank.」として
出題される**ので、[ツール]→[空のカードを削除] で消すこと。この66枚は
カード1とまったく同じ内容だった重複カードなので、消して差し支えない。
実行後の同期は**「アップロード」**を選ぶこと(フィールド追加=スキーマ変更)。

**あわせてプロンプト(`docs/shared/grammar_multi_prompt.txt`)も直した**:

- 規則6: `answer` は**必ず完全な英文1文**にする(選択問題で空所に入る語だけを
  返すのを禁止)。実データで「(A) involved」のような語句だけの回答が出ており、
  カード1の裏・カード3の表がどちらも文脈を失っていたため。
- 規則7: `examples` を必須にし、**`<b>` で囲むのは任意ではなく必須**だと
  明記した。囲まれないと穴埋めカードが作られず、実際に61枚しか生えて
  いなかった(155ノート中)。
- JSON出力例の `answer` と `examples` も、規則が要求する形の**具体値**に
  置き換えた(プレースホルダーのままだと従わない、という前例があるため)。
...
- **【2026-08-19】ローカルのTTSツール(既存apkgへの音声付け)を追加した**。
  実装・自動テストは完了(下記「ローカルのTTSツール」を参照)。残りは
  (a) 片桐の実機で `apkgにTTS音声を付ける.bat` から起動して475件の本番実行、
  (b) それが問題なければ `tts_gui.py` をこの機能だけに単純化する作業。

- **【提案済み・片桐の判断待ち】2026-08-05のバグ検査で挙がった改善案の残り**:
  同日の検査で見つかったバグ8件は修正済み(下記「2026-08-05のバグ修正」)。
  併せて提案した改善案6件のうち、**5件は同日実装済み**
  ...
  - **デスクトップ版とWeb版のストック統合は「見送り」で確定(2026-08-05)**。
    ...
    - 形式の隔たり: 習熟用は デスクトップ`source_key: ["dailyconv","<id>"]`
      に対しWebは`source_kind`/`source_topic`(平坦)。変換自体は機械的。
    - **本当の難所は「出力済み」の考え方の違い**: デスクトップは
      「出力したらpendingから消して`exported_keys`にキーだけ残す」、Webは
      「`exported_at`を付けて項目は残す」。揃えるにはどちらかの挙動を
      変えるか、変換時に情報を捨てる判断が要る。
    - デスクトップ側に**削除の記録(tombstone)が無い**ため、そのまま繋ぐと
      デスクトップで削除した項目がWebから復活し続ける。デスクトップの
...
- **【完了】Googleログインを長持ちさせる対応(2026-08-05)**:
  ...
  - 同意画面は片桐の選択で「テスト」ステータスのままのため、
    **リフレッシュトークンは7日で失効する**(週1回のログインは残る。
    1時間ごとよりは大幅な改善)。無期限にしたい場合は「本番環境」への
    公開が別途必要で、その場合は初回に「確認されていません」警告が出る。
    **2026-08-12前後に「またログインを求められる」と報告があった場合、
    不具合ではなくこの7日失効である可能性が高い**(まずそれを疑うこと)。
  - Worker URLを空欄に戻せば従来方式(B)へ即座に戻せる。Workerが落ちた・
    Cloudflare側の障害といった場合の退避路として覚えておくこと。
...
- **リダイレクトURIは2つ登録してもらうこと**(2026-08-05): `redirectUri()`は
  ...
  なる。ブックマークやホーム画面のアイコンがどちらの形かは分かりにくく、
  片方しか登録していないと「PCでは入れるのにスマホでは`redirect_uri_mismatch`」
  という切り分けにくい失敗になる。`worker/README.md`に両方を記載済み。
- **401時のアクセストークン自動リトライ**(`sheetsFetch`、2026-08-05追加):
  Worker方式はリフレッシュトークンを持っているのに、以前は401のたびに
  「もう一度同じ操作をしてください」と手動の再実行を求めており、長い操作
  (添削→シート追記→習熟用生成)の途中で期限が切れると最初からやり直しに
  なっていた(「ログインが長持ちする」という(A)の狙いが体験として完成して
  いなかった)。`sheetsFetch`が401を受けたら`clearAccessToken()`→
  `refreshAccessToken()`→**新しいトークンで1回だけ再送**する。
  - 取り直す条件は`canSilentlyReauth()`(Worker URLが設定済み **かつ**
    リフレッシュトークンを保存済み)。**従来方式(B)では自動リトライしない**
    ——取り直しに同意画面・ポップアップが要るため、利用者が操作していない
    のに勝手にポップアップが出ることになる。
  - `getAccessToken()`が`lastAuthOptions`(workerUrl/clientId)をモジュール
    変数に覚えており、`sheetsFetch`はそれを見て判断する(API部の関数は
    accessTokenを引数で受け取る純粋な関数、という既存の設計を崩さないため)。
  - リトライは**1回だけ**。それでも401なら従来どおり`SheetsAuthError`を
    投げる(無限ループにしない)。`describeSheetsError`の401メッセージも
    「自動での取り直しに失敗した状態」を前提に、手動での再ログインを
    案内する文面へ変更した。
- **`isSignedIn()`には現在のWorker URLを渡すこと**(2026-08-05): 引数無しだと
  「Worker URLを消した/変えた後に残っているリフレッシュトークン」だけで
  ログイン済みと表示してしまい、実際は方式(B)へ落ちるため表示と挙動が
  食い違う。リフレッシュトークンはWorker経由でしか使えないため、Worker URLが
  空のときはアクセストークンの有無だけで判定する。
- **403 `ACCESS_TOKEN_SCOPE_INSUFFICIENT`は「シートの共有設定」の問題ではない**
  (2026-08-13、実機で発生)。Googleの同意画面は**権限ごとにチェックボックスが
  分かれており**、スプレッドシートの項目をオフのまま「続行」を押すと
  `openid email`だけのトークンが発行され、シートを読む段階でこの403になる。
  同意画面が「テスト」ステータスのためリフレッシュトークンは7日で失効する
  ——つまり**この取り違えは週1回のログインのたびに起こりうる**。
  - **自力では復帰できない形だった**: 発行されたリフレッシュトークンも同じ
    足りないスコープを持つため、無言の取り直しが延々と同じ403を返す。
    しかも403は`sheetsFetch`の401自動リトライの対象外。そこで
    `isScopeInsufficient()`(`docs/lib/sheets.js`)で判定し、
    **`signOut()`で保存済みリフレッシュトークンごと捨ててから
    `SheetsAuthError`を投げる**(次のログインで同意画面をやり直させる)。
  - `describeSheetsError`の403分岐は、汎用の`insufficient`/`permission`より
    **前に**このスコープ判定を置くこと。順序を戻すと、以前と同じ
    「シートの共有設定とアカウントを確認してください」という**対処しても
    直らない案内**に戻る(実際にこれで片桐を共有設定の確認へ誘導してしまった)。
  - 回帰テストは`test_sheets.mjs`の[2]。

#### 新規デバイスの設定を最小化する仕組み(2026-08-05追加)

片桐から「APIやスプレッドシートIDなどを全部個別に入力するのが、新規デバイスで
かなり面倒」との指摘を受けた対応。以前は新しいPC・スマホで使い始めるたびに
**最大8項目**を手入力する必要があった。現在は**ページを開いてログインを
押すだけ**で使える。

- **鶏と卵の問題**: 同期でまとめて配れば済むように見えるが、
  「Worker URL」と「スプレッドシートID」だけはログイン・同期より前に必要なため
  同期では解決できない。この2つをどう配るかが設計の要点だった。
- **Worker URL はアプリに埋め込む**(`DEFAULT_OAUTH_WORKER_URL`、`docs/app.js`)。
  公開しても害が無い(URLを知っているだけでは秘密は取り出せない)。
  ⚙設定の入力欄で上書きでき、Cloudflare障害時に従来方式へ退避できる。
- **残りは Worker の `GET /appconfig` が配る**(`worker/src/index.js`)。
  スプレッドシートID・シート名・Gemini APIキー・Cloud TTS APIキーを
  Workerのシークレットとして登録しておき、ログイン後に受け取る
  (`sheets.fetchAppConfig` → `app.ensureAppConfigLoaded` → `applyRemoteAppConfig`)。
- **なぜ`_AppSync`(スプレッドシート)ではなくWorkerに置くのか**: あのシートには
  デスクトップ版のサービスアカウントが編集者権限を持っており、その鍵JSONは
  Google Drive同期フォルダに仮置きのまま(「今後の拡張候補」参照)。課金対象の
  APIキーは秘密情報用に作られた場所(Cloudflareのシークレットストア)に置く。
  副次的な利点として、**キーの入れ替えがWorker側の1箇所で済む**。
- **`/appconfig` は必ず本人確認を通してから返す**(`verifyAccessToken`)。
  `/config`(クライアントIDのみ)と違い秘密を返すため。CORSはブラウザ内の
  JavaScriptしか縛れず、curl等の直接呼び出しは防げない(実際に本番の
  `/config`をcurlで叩けることを確認済み)。Googleの`tokeninfo`で
  (1)トークンが有効 (2)**このアプリのクライアントID向けに発行された**
  (aud/azpの照合) (3)メールが`ALLOWED_EMAIL`と一致し確認済み、の3点を検証する。
  - **`ALLOWED_EMAIL`未設定なら誰にも返さない(fail closed)**。ここが開いていると
    「ログインさえすれば誰でもAPIキーを取得できる」状態になる。
  - このためスコープに`openid email`を追加した(`OAUTH_SCOPE`、`docs/lib/sheets.js`)。
    **スコープを増やすと同意画面がもう一度出る**(追加時の一度きり)。
    これが欠けるとtokeninfoがメールを返さず本人確認できないため、
    `test_sheets.mjs`でスコープに含まれることを固定してある。
  - Worker側で未登録の項目は`null`を返し、**アプリ側は反映しない**
    (端末で手入力した値を消さないため)。
- **「画面で調整する設定」はWorkerに入れない**(Geminiモデル名・TTS音声/言語/
  音量ゲイン/日本語除外)。Workerに入れると変更のたびに再デプロイが必要になる。
  これらは秘密ではないので`_AppSync`の`app_settings`行で同期する
  (`SYNCED_SETTING_FIELDS`/`collectLocalSettings`/`mergeRemoteSettings`)。
  ストックと違い**項目ごとのマージはせず**`updated_at`が新しい方をまとめて
  採用する(数個の独立した値であり、複雑なマージに見合う利点が無いため)。
- **チェック順序の変更に注意**: `requireSheetsAccess`/`runSync`/
  `autoPullOnStartup`は、以前「スプレッドシートIDの有無 → ログイン」の順で
  見ていたが、IDがログイン後に配られるようになったため**順序を逆にした**。
  この順序を戻すと、新しい端末での初回だけ必ず失敗する。
- **`/appconfig`の失敗を黙って握り潰さないこと(2026-08-19修正)**: 「スマホの
  Chromeでログインしても必要なAPIキーなどが自動で入ってこない」と報告された。
  調査の結果、**Worker側は正常**(本番の`/appconfig`はダミートークンに401を
  返し=`ALLOWED_EMAIL`登録済み、`wrangler secret list`で`SPREADSHEET_ID`/
  `GEMINI_API_KEY`/`TTS_API_KEY`も登録済みであることを確認)。問題は
  `ensureAppConfigLoaded`(`docs/app.js`)側にあり、失敗しても
  `console.warn`だけで**画面には一切出さず**、しかも`appConfigLoaded = true`を
  立てて諦めていた。この2つが重なると次のようになる:
  - 利用者から見ると「ログインは成功したのに設定が入らない、理由も分からない」
    という手詰まりになる(原因はどれも端末側で対処できるものなのに、
    どれなのかが分からない)。
  - 起動直後の1回が失敗すると、そのページを開いている間は
    **「🔄 同期」を押しても DailyConversation を使っても二度と取りに行かない**。
    再ログインしても直らないため、ページを開き直すしかなかった。
  - 修正: 失敗時は`appConfigLoaded`を立てず、代わりに`appConfigFailed`を立てる。
    **このフラグを見るのは`autoPullOnStartup`(`{auto: true}`で呼ぶ)だけ**で、
    片桐が押した操作(`runSync`/`requireSheetsAccess`)からは何度でも再試行する。
    失敗理由は`appConfigErrorMessage()`で通知バナー(`header-auth-status`、
    `data-log`付きなので⚙設定の「ログ」にも残る)へ出す。
    `onHeaderSignOut`は両フラグを倒す(別アカウントで入り直したときに、前の
    アカウントでの結果を引きずらないため)。
  - **スマホで最も疑わしいのは「別のGoogleアカウントでログインしている」**
    (Chromeの既定アカウントがPCと違うことが多い)。この場合Worker側は
    `ALLOWED_EMAIL`と一致しないトークンを403で拒否する——シートの共有設定や
    APIキーの問題ではないので、403の文言はアカウントの確認へ誘導している。
    次点は**ブラウザが古いapp.jsをキャッシュしている**ケース(⚙設定の先頭に
    出る版で切り分ける)。
...
- **`.apkg`の生成に成功してから「Anki出力済み」をマークする**(デスクトップ版
  ...
  二度と一覧に出てこなくなる。
- **`onDailyExport()`の後処理は生成本体とは別の`try`に分けてある**
  (2026-08-05修正)。以前は同じ`try`に入っており、`markRowsAsExported`が
  401やネットワークで失敗すると「.apkgの生成に失敗しました」と表示されて
  いた。実際にはファイルはダウンロード済みなので、片桐が「失敗した」と
  読んで再出力する導線に乗りやすく、無駄なTTS/Gemini消費や二重マークに
  つながる。**後処理側のエラーメッセージは「apkgは出力済み」を先に伝える
  こと**(再出力しなくてよいと分かるように)。
...
- **DailyConversationの3つ目のフィルター**: シート側の「Anki出力済み」列
  ...
  状態に関わらず必ずこの記録を行う。「出力済みの印を外す」ボタンは
  このローカル記録だけを消し、シートの「Anki出力済み」列には一切触れない。
- **DailyConversationの出力対象も「まだ出力していない行だけ」に揃えた
  (2026-08-06)**: それまで`onDailyExport()`は`dailyPendingRows`(表示
  フィルター適用**前**)を丸ごと出力しており、**一度出力して一覧から
  消えた行が毎回また出力され、TTSも作り直されていた**(「出力済み(この
  ブラウザで記録)を隠す」が既定ONなので、通常運用でほぼ必ず該当した。
  Anki側はguidが同じで上書き更新されるため実害は無いが、時間とAPI料金が
  毎回かかる)。画面の説明「ここに出ているものが③の出力対象です」とも
  食い違っていたため、単語/AIに質問の`onExport`と同じく
  `loadExportedIds()`で除いてから`processSheetRows()`に渡す方式に変更。
  除いた件数は完了メッセージに出す。もう一度出したい場合は②の
  「出力済みの印を外す」で戻せる(この逃げ道が実際に効くことも
  `test_web_ui.mjs`の[23]で固定した)。
...
「文章を載せたら、**それが正しい場合に限り**、同じ文法の別の文章を2つ生成して
カードにしたい」との要望への対応。習熟用タブに初めて直接の入力欄ができた
(従来は「AIに質問」の4問目とDailyConversationの添削からしか増えなかった)。

- **正誤の判定は`correctEnglishText()`を使い回す**(`onShuujukuGenerate`、
  `app.js`)。`category === '誤りなし'`の文だけを`generateShuujukuItemFromSentence()`
  (`lib/gemini.js`、プロンプトは`docs/shared/shuujuku_sentence_prompt.txt`)へ
  渡す。**ここに独自の正誤判定を作らないこと**——添削の`system_instruction`は
  Apps Script(Googleフォーム経路)と意味的に同一に保つルールがあり、独自判定を
  足すと3つ目の採点基準が生まれて「DailyConversationでは誤り、習熟用では正しい」
  という食い違いが起きる。副産物として、弾いた文の理由(explanation・カテゴリ)を
  そのまま表示できる。
- **`consolidateNoErrorCorrections()`は通さないこと**(実装中に踏みかけた罠)。
  あれは「誤りなし」の複数文を`original`を改行で連結して**1行にまとめる**関数で、
  シートに「1送信=1行」で書き込むためのもの。ここで通すと、正しい文を3つ
  入力しても「3文が連結された1枚」のカードになる。
- **examples は「入力文 + 生成された別の例文2つ」の3つ**(片桐の指示で元の文脈も
  残す)。**入力文はAIの出力からではなく、渡した文字列をそのまま先頭に置く**
  ——AIに「入力文もそのまま返して」と頼むと言い換え・大文字小文字の変更の
  余地が残るため、日本語訳(`sentence_ja`)だけを受け取って組み合わせる。
- `source_kind`に3つ目の値`'sentence'`が増えた(既存は`'chat'`/`'dailyconv'`)。
  `source_topic`は正規化した入力文。guidは
  `compound(prefix='shuujuku', keys=[source_kind, source_topic])`なので
  **カード定義もapkg側も無変更**で、重複検出(`source_topic`基準)・同期も
  そのまま効く。
- 誤りのある文は**カードにせず、DailyConversationタブの入力欄へ転記する**
  (`transferToDailyInput()`、2026-08-06に片桐の指示で告知だけ→自動転記に変更)。
  あちらに「添削→シート追記→習熟用カード自動生成」の導線が既にあり、ここに
  「添削後の文で作る」まで持たせると機能が重複するため、誤り文の受け皿は
  あちらに寄せる。あちらは元々「自分の誤りを記録する場所」なので、
  誤り文がそこに入るのは自然な流れという判断。
  - **タブの自動切り替えはしない**(結果メッセージを読んでいる最中に画面が
    飛ぶのを避ける)。転記した旨をメッセージで伝えるに留める。
  - **既存の入力内容は消さずに追記**する(打ちかけの文を失わないため)。
    ログインは不要(テキストを入れるだけ)。
  - **二重転記の防止は2つ必要**(2026-08-06のレビューで判明。当初は
    「入力欄の現在の中身に同じ**行**があるか」だけを見ており不十分だった。
    根本原因は、転記済みかどうかの記憶を入力欄の現在値だけに頼っていたこと):
    1. `transferredToDaily`(このセッション中に転記した文の集合)。添削に
       成功すると`onDailyCorrect`が入力欄を空にするため、これが無いと
       習熟用タブに残った誤り文でもう一度生成したときに**同じ文が再び転記され、
       シートに重複行ができる**(無駄なGemini呼び出しにもなる)。ページを
       再読み込みすれば消えるが、その時は両方の入力欄も空なので取りこぼさない
       (localStorageに残すと、意図して消した文を二度と転記できなくなる)。
    2. 行単位ではなく**正規化した本文への部分一致**で判定する。このタブは
       「複数文を1行にまとめて入力」を明示的に認めているため、行比較では
       その中に含まれる同じ文を検出できない。
  - **`onDailyCorrect`は入力欄を無条件に空にしないこと**(同レビューで判明)。
    添削は数十秒かかることがあり、その待ち時間中に習熟用タブが転記していると、
    それごと消えてしまう(習熟用タブ側は「転記しました」と表示したまま)。
    転記は末尾への追記なので、**送った分は先頭に残っている**——その分だけを
    取り除く。判別できない形(待ち時間中に片桐自身が編集した等)なら、
    消さずにそのまま残す(勝手に消して入力を失わせるより安全)。
- ボタン名は「カードを生成する」、その直下に赤字(`.caution`)で「正しい場合のみ
  生成する」旨を常設する(片桐の指示。押す前に条件が分かっていないと
  「入力したのにカードが増えない」と誤解されるため)。
- 複数文をまとめて入力できる(構造化出力が文ごとに分割する)。**判定は
  何文でも1回のAI呼び出し**。カード生成も2026-08-06にバッチ化したので、
  入力が何行でもAI呼び出しは**最大3回**(判定1回 + 英文のカード生成1回 +
  単語・表現のカード生成1回。10件を超えると`BATCH_SIZE`ごとに分かれる)。

#### 単語・句動詞など「完全な文ではないもの」の入力(2026-08-06追加)

片桐から「習熟タブで単語や句動詞など、完全な文ではないものが入力されたとき、
その表現を同じ文法構成で使ったサンプル英文を作って出力したい。ただし文法構造が
同じでも内容は別のものにしてほしい」との要望への対応。

**それまでの問題**: 単語・句動詞をそのまま`correctEnglishText()`にかけると、
文ではないので必ず「誤り」と判定され、カードにならずDailyConversationタブへ
転記されてしまっていた(この機能を入れる前の挙動)。

- 入力を行ごとに振り分ける(`looksLikePhrase()`、`docs/app.js`)。
  **文末に`.`/`!`/`?`があれば「文」、無ければ5語以下(`PHRASE_MAX_WORDS`)を
  「表現」**とする。語数の条件があるのは、句読点を打たずに文を書いても
  長ければ文として扱われるようにするため。
- **なぜAIに分類させずローカルで判定するのか**: 添削へ送る**前**に振り分ける
  必要があり(送ってしまうと必ず「誤り」になる)、AIに分類させると呼び出しが
  1回増えて、呼び出し回数を減らしてきた方針と逆行するため。取り違えは
  起こりうるので、**結果メッセージにどちらとして扱ったかを必ず出す**
  (気づいたら文末に「.」を付けて出し直せる。この逃げ道は画面の説明にも
  書いてあり、`test_web_ui.mjs`の[29]で固定してある)。
- プロンプトは`docs/shared/shuujuku_phrase_prompt.txt`(バッチ形式)。
  `generateShuujukuItemsFromPhrases()`(`docs/lib/gemini.js`)が使う。
- **入力表現は例文に流用しない**。文の経路(`generateShuujukuItemsFromSentences`)
  は入力文をそのまま1つ目の例文に置くが、単語・句動詞は文ではないので
  例文にできない。代わりに**その表現を含む例文を生成させる**(本数は下記)。
- 「同じ文法構造でも内容は別のもの」という要望は、プロンプトで
  「例文どうしは文法構成が同じでも内容(主語・場面・話題・語彙)をそれぞれ
  まったく別にすること」と明示し、**悪い例**(主語と目的語だけ差し替えた3文)
  まで書いて指示してある。
- **入力した単語・句動詞は、何件でもまとめて1枚のカードにする**
  (2026-08-06。当初は「1件=1枚」で実装したが、実機で2件入れて2枚できたのを見て
  片桐から「1つのカードで、例文の数が語句の数によって変わる想定だった」との
  指摘を受けて変更した。**英文の経路は従来どおり1文=1枚**)。
  例文数は`phraseExampleCount(件数)` = `min(max(3, 件数 + 2), 10)`で、
  1件→3 / 2件→4 / 3件→5 / … / 8件以上→10。
  プロンプトで「入力した表現が全体を通して必ず1回以上登場すること」を指示する。
  - **上限を5から10へ引き上げた**。5だったのは「N枚×最大5例文」で応答が
    出力トークン上限を超える懸念からで、1枚にまとめた今その根拠はほぼ無い。
    むしろ**5のままだと6件以上入れたときに一度も例文に出てこない語句が出る**
    ——入力したものが黙って消えるのは、このアプリで一貫して避けている挙動。
  - 8件を超えると全語句を載せきれないため、`phraseExamplesMayOmit()`が
    結果に「一部の表現が例文に出てこない可能性があります」と警告する。
  - **`pattern`/`meaning`に改行やHTMLタグを入れさせないこと**。
    `shuujuku.js`の`renderItem`が`esc()`してから描画するので、`<br>`は
    そのまま文字として表示されてしまう。複数の表現は` / `区切りで1行に並べる
    ようプロンプトで指示している。
  - `source_topic`は各語句を正規化して**並べ替えてから**` | `で連結する
    (同じ語句を順番だけ変えて入れ直したときに、別カード扱いで二重に
    できないため)。
  - 出力はJSON配列ではなく**オブジェクト1つ**。1枚しか作らないので
    `alignBatchResults`によるindex対応付けもチャンク分割も不要になった。
  - テスト(`test_web_ui.mjs`の[29])は**送信したプロンプトの中身**と
    **カードが1枚にまとまること**を検証する。
...
- **区切りは改行のみ**(2026-08-06、片桐の選択)。カンマ・スラッシュを区切りと
  ...
  - ただし`give up, look forward to, put off`のように1行へ並べてしまうことは
    ...
    できない**という一番分かりにくい失敗になる。そのため
    `looksLikePhrase`は区切り記号(`,` `、` `/` `・`)を含む行を語数に
    関わらず「表現」として扱い、少なくとも1枚は作る。そのうえで
    `looksLikeMultipleEntries`が結果に「1行に複数の語句が並んでいるように
    見えます。区切りは改行のみです」と案内する。
  - この扱いなら**行き止まりが無い**: 誤って並べた場合は案内を見て入力し直せる
    し、カンマを含む正しい成句はそのまま1枚になる。長い文にカンマがあっても
    文末に「.」を付ければ従来どおり添削へ回る。
- デスクトップ版には入れていない(直接入力の機能自体がWeb版のみのため)。

#### すべて大文字の入力の自動修正(2026-08-06追加)

片桐から「習熟用タブですべて大文字だと誤り扱いになる。スペルが合っているなら
小文字に自動修正してカード生成を進めたい」との要望への対応
(補足で「小文字に自動修正というのは**カード生成時の話**」)。

- **添削へ送る前にこちら側で表記を直す**(`isAllUpperCase` /
  `normalizeAllCaps`、`docs/app.js`)。他の手が使えないための選択:
  添削のカテゴリは「文法 / 語彙 / 自然さ / 誤りなし」の4つだけで
  (`docs/shared/correction_response_schema.json`)、**大文字小文字の表記だけを
  表すカテゴリが無い**ため「大文字が理由の誤りだけ見逃す」という選び方が
  できない。かつ`system_instruction`はApps Script(Googleフォーム経路)と
  意味的に同一に保つルールがあるので、添削側に手を入れる選択肢も無い。
  送る前に直せば、スペルが正しければ素直に「誤りなし」になる。
- **単純な`toLowerCase()`では駄目**。文の場合
  `I HAVE BEEN WORKING HERE SINCE 2020.` → `i have been working here since 2020.`
  となり、**文頭の小文字と一人称の`i`という別の誤りを新たに作ってしまう**。
  `normalizeAllCaps(text, {asSentence})`が、文として扱う行では文頭
  (および`.`/`!`/`?`の後)を大文字に戻し、`\bi\b`を`I`へ直す
  (`\b`は`'`を語境界とみなすので`i'm`/`i've`/`i'll`/`i'd`もこの1行で直る)。
  単語・句動詞は小文字のままでよいので`asSentence: false`。
- **固有名詞は復元できない**(`TOKYO` → `tokyo`)。ローカルの規則では判別
  できないため、直した内容を必ず結果メッセージに「元 → 直した後」の形で
  出して確認できるようにしてある。
- **入力欄そのものは書き換えない**(打った内容が勝手に変わる驚きを避ける)。
  直した表記が使われるのはAIへ送る本文とカードの内容だけ。
- 小文字が1文字でも混ざっていれば手を触れない(`Give UP`はそのまま)。
  意図的な大文字表記を勝手に潰さないため。
- 単語側の`source_topic`は元々小文字へ正規化しているので、`GIVE UP`と
  `give up`のguidは元から同じ。この変更で重複カードが増えることはない。
- テストは`test_web_ui.mjs`の[29]。**添削・カード生成へ実際に送った本文**を
  検証する(直したつもりで送っていない、という取り違えを防ぐため)。
- デスクトップ版には入れていない(片桐の指示で「取り敢えずWeb版のみ」)。
  プロンプトは共有フォルダに置いてあるので、後から移植しても不一致は起きない。
- テストは`test_web_ui.mjs`の[28]。

### .apkg出力の同時実行を禁止(2026-08-06)

片桐から「**AIに質問でapkgを出力しながら習熟用タブでも同時に出力すると、
AIに質問の方がちゃんと出力されないのに『出力済み』になる**」と報告された。
調査の結果、原因の異なる2つの問題が重なっていた。

- **(A) ダウンロードの成否を確認する手段が無い**: ブラウザは1つのページが
  短時間に複数のファイルをダウンロードしようとすると、2つ目以降を確認
  プロンプトで止めたり黙って破棄したりする(Chromeの「複数のファイルの
  ダウンロード」許可、iOS Safariの不安定さ)。`downloadBlob`の`a.click()`は
  **破棄されても例外を投げない**ため、アプリからは成功と区別が付かず、
  そのまま`exported_at`を付けてしまう。**ダウンロード完了を知るブラウザAPIは
  無い**ので、そもそも同時に走らせないのが唯一の確実な対処。
  `beginApkgExport()`/`endApkgExport()`(`app.js`)で、出力中は
  `EXPORT_BUTTON_IDS`の4つ(単語/AIに質問/習熟用/DailyConversation)を
  まとめて押せない状態にし、モジュール変数`apkgExportInProgress`でも弾く。
  **呼び出し側は`beginApkgExport`がtrueを返したら必ず`finally`で
  `endApkgExport()`を呼ぶこと**(でないと以後どのタブからも出力できなくなる)。
  - **`disabled`属性は使わず`aria-disabled="true"`+CSS(`button[aria-disabled]`)で
    見た目だけ無効に揃えている(2026-08-06、片桐の指示)**。当初は`disabled`に
    していたが、「押せなくなるのは良いが、**その場合のみ**同時出力できない旨の
    メッセージを出してほしい」との要望を受けて変更した。ブラウザは無効な
    formコントロールへのクリックを**イベントとして配送しない**(祖先にも
    上がらない)ため、`disabled`のままでは押しても本当に何も起きず、理由を
    伝える機会が無い(=ただの反応しないボタンに見える)。`aria-disabled`なら
    クリックを受け取れるので、ハンドラ側の`rejectWhileExporting()`が
    そのタブのステータス欄に理由を出せる。支援技術にも「操作できない」と
    正しく伝わる。
  - `rejectWhileExporting()`は各出力関数の**先頭**で呼ぶこと。「カードが
    ありません」等の事前チェックより後ろに置くと、たまたま条件を満たさない
    タブを押したときに出力中であることが伝わらない。
- **(B) `onExportShuujuku`が出力中に増えた項目まで消していた**(同時に発見・
  修正): 後始末で`shuujukuStock`(現在値)をそのまま空にしていたが、習熟用
  ストックは「AIに質問」の4問目生成・DailyConversationの添削から**非同期に
  増える**。TTS付きの生成は数分かかることがあり、その間に増えた項目まで
  「出力済み」として削除し、**打ち消し記録にまで載せて他の端末からも消す**
  データ消失の恐れがあった(バックアップからしか戻せない)。開始時点の
  スナップショット`targets`を出力対象とし、後始末も参照比較で
  `targets`のぶんだけ取り除くよう修正した。単語/AIに質問の`onExport`は
  元からこの形(`pendingItems`のSetで参照比較)なので変更していない。

テストは`test_web_ui.mjs`の[27]。(a)出力中は全タブのボタンが`aria-disabled`に
なること、(b)`disabled`属性は**付けない**こと(付けるとクリックを検知できず
理由を出せないため、退行防止として明示的に固定してある)、(c)押すと理由が
その場に表示されること、(d)拒否された側のストックが消えないこと、
(e)終了後は再び押せるようになり、直列にやり直せば出力できること、を確認する。
**`disabled`を使っていた最初の実装では「押しても何も起きない」ため、
クリック後にメッセージを期待したテストが失敗した**——この失敗が、そもそも
利用者にも理由が伝わらないことを示していた。

### 2026-08-06の文言整合性の点検(まとめ)

片桐から「ソフト内の文言がシステムの挙動と整合しているか確認してほしい」との
依頼を受け、Web版の全画面文言を実装と突き合わせた。見つかった7件のうち、
1件は**文言ではなく挙動のほうを直した**(上記のDailyConversationの出力対象)。
残りは文言修正:

- DailyConversationタブの「事前に⚙設定でクライアントID・スプレッドシートID・
  シート名の設定も必要」→ 2026-08-05の「新規デバイスの設定を最小化する仕組み」
  以降は不要になっており、⚙設定冒頭の「ふつうは何も入力する必要はありません」と
  矛盾していた。
- 習熟用(音読)タブ①「『AIに質問』タブで質問を送信すると追加される」→
  DailyConversationの添削経由(`generateShuujukuCandidatesFromRows`)が
...
- TTSの「自動調整」→「引き上げます」と書いてあったが、
  ...
  音声では下げる。
- ⚙設定「ログインを押すと自動設定されます」→ `ensureAppConfigLoaded`の
  呼び出し口は`autoPullOnStartup`/`runSync`/`requireSheetsAccess`の3つで、
  「起動時に自動で読み込む」をOFFにしていると同期かDailyConversationを
  使うまで反映されない。
- 「出力済みを削除」の確認文「復元できません」→ ⚙設定のバックアップから
  復元できる(設定側は「実行前に書き出しを推奨」と案内していた)。
- Worker URLの説明に、同意画面が「テスト」ステータスのため
  **リフレッシュトークンが7日で失効する**ことを追記した(「またログインを
  求められる」と報告される前に自己解決できるようにするため)。

**ボタンの名前と並びも整理した**(片桐から「『すべて削除』はログも消えるのか、
どのボタンがどう作用するか分かりづらい」との指摘)。原因は、カードごと消える
操作と印だけ変える操作が同じ行に並んでいたこと、および「リセット」「削除」
...
- **「出力済み」の記録の持ち方がタブで違う**点は、この機会に明記しておく:
  ...
  「すべてのカードを削除」すると印も一緒に消える(独立した「履歴」は
  存在しない)。DailyConversationだけは行の実体がシート側にあるため、
  出力済み記録が独立したlocalStorage(`anki_tool_daily_exported_ids`)に
  ある。旧名「出力済み履歴をリセット」は、前者にも独立した履歴が
  あるかのように読めるのが分かりづらさの一因だった。

### Gemini APIの使用状況カウンタ / まとめて生成(バッチ)(2026-08-06追加)

片桐の環境で `gemini-3.5-flash` の無料枠(**20リクエスト/日**)にぶつかり、
「使用しているAPIの使用率を見えるようにするにはどうすれば良いか」との
相談を受けての対応。続けて**「AIの上限はすぐに変更になることが多いので、
カウンターだと無意味になる可能性が高い」**という重要な指摘があり、それを
設計の出発点にしてある。

#### 上限値をソースに書かない(lib/quota.js)

陳腐化するのは**上限値だけ**で、「今日何回呼んだか」という数字自体は
古くならない。そこでこの2つを分離した:

- 回数 … `docs/lib/quota.js` が「日付 × モデル」ごとに数える(常に正しい)
- 上限 … **Googleが429の応答本文で自分から教えてくれる値**
  (`details[].violations[].quotaValue`)を学習する

`postGeminiRequest`(`docs/lib/gemini.js`)が429を受けるたびに
`learnDailyQuotaLimit()`を呼ぶので、Googleが上限を変更しても、次に一度
ぶつかった時点で自動的に新しい値へ入れ替わる。**アプリ側には上限の数字を
一切持たせないこと**(持たせた瞬間に、片桐が懸念したとおり陳腐化する)。

- **日次(quotaIdに`PerDay`を含む)の上限だけを学習する**。分あたりの
  レート制限(RPM)の値を日次の分母として覚えると、実際よりずっと小さい
  分母が表示され誤解を招く。
- **リトライも1回として数える**(`recordRequest`は`fetch`のたびに呼ぶ)。
  Google側は失敗したリクエストも割り当てとして数えるため、成功した論理
  呼び出しだけを数えると実際より少なく出る。
- **日付は太平洋時間で切る**(`quotaDateString()`)。無料枠のRPDは太平洋時間の
  深夜にリセットされるため、端末のローカル日付で切ると「日付が変わったのに
  Google側のカウンタはまだ戻っていない」という食い違いが毎日起きる。
- カウントは日付が変わると捨てるが、**学習した上限は持ち越す**(毎日
  ぶつかり直さないと分母が出ない、では意味がない)。
- 表示は⚙設定の「Gemini APIの使用状況」(`renderQuotaUsage()`、設定を
  開くたびに描き直す)と、生成完了時のメッセージ末尾(`quotaSuffix()`)。
  **上限を観測していないモデルでは分母を出さない**——推測の数字を出すのは、
  古い上限を信じさせるのと同じことなので。
- `globalThis.localStorage` へのアクセスはすべて try/catch で包み、
  モジュール読み込み時には触らない。`tools/test_gemini.mjs` などが
  jsdom無しの素のNodeで`gemini.js`を読み込むため。
- テストは`tools/test_quota.mjs`(`npm run test:quota`)。

#### 消費そのものを減らす(まとめて生成)

上限がいくつであれ効くので、こちらのほうが対策として本質的。以前は
「単語N件 → N回」「添削結果N行 → N回」と1件ずつ直列に呼んでいた。

- `generateVocabCards({pairs, ...})` / `generate_vocab_cards_from_words(pairs, ...)`
- `generateShuujukuItemsFromRows({rows, ...})` / `generate_shuujuku_items_from_rows(rows, ...)`

**プロンプトはWeb版と共有しているため、片方だけをバッチ化することはできない**
(`docs/shared/word_card_prompt.txt` /
`shuujuku_dailyconv_prompt.txt` は入力が複数件・出力がJSON配列という前提に
書き換わっている)。そのため今回はデスクトップ版(`gemini_client.py` /
`tts_gui.py`)も同時に変更してある。プロンプトを別ファイルに分けて
片方だけバッチ化する案は、CLAUDE.mdが繰り返し警告している「片方だけ直して
不一致になる」状況を自分から作ることになるので採らなかった。

- `BATCH_SIZE = 10`(両実装で同じ値に保つこと)。全件を1回に詰めないのは、
  件数が増えると応答が出力トークン上限で途中で切れ、**そのバッチが丸ごと
  失敗する**ため。10件ずつなら被害がそのバッチに限定される。
- **`index`(1始まり)で対応付ける**(`alignBatchResults` /
  `_align_batch_results`)。配列の位置をそのまま信じないのは、モデルが
  1件飛ばす・順序を入れ替えることがあり、そうなると**別の単語の解説が
  付いたカードが静かに出来上がる**ため(内容が入れ替わっても字面だけでは
  気づきにくい)。`index`が使えない要素だけ、埋まっていない位置へ順に詰める。
- 戻り値は入力と同じ長さで、**生成できなかった件は`None`/`null`**
  (詰めない)。どの単語・どの行が落ちたかを利用者に伝えられるようにするため。
  個別のリトライはしない(それをやると呼び出し回数を減らした意味が薄れる)。
- バッチ全体が失敗した場合(APIキー不正・上限到達・応答が壊れている等)は
  個別の件に切り分けられないので、まとめて1件の失敗として扱う。
- 効果: 「単語10語 + 習熟用5行 + AIに質問3回」が **21回 → 8回**。
- **「AIに質問」の3問生成と習熟用4問目はバッチ化していない**。前者は1つの
  質問から3件を作る呼び出しで既に1回、後者も1質問=1件なので、まとめる
  対象が無い。
- 回帰テスト: `test_web_ui.mjs`の[3]で「2行の入力でも呼び出しは1回」を
  固定してある(ここが2に戻っていたら元の方式へ退行している)。
  モック側は`fakeBatchArray()`がプロンプト内の`[1]` `[2]` …を数えて
  要求件数ぶんの配列を返す(app.js側が組み立てる書式に依存するので、
  書式を変えたらここも追従すること)。

#### モデル選択を`<select>`に変更(2026-08-06)

片桐から「モデル欄は入力した文字に適合する一覧しか出てこない。何を入力して
いても一覧を全部表示したい」との指摘。原因はアプリの不具合ではなく
**`<datalist>`の仕様**で、`<input list>`の候補は入力欄の文字で必ず絞り込まれる
(「gemini-3.6-flash」と入っていると候補がその1件だけになり、別のモデルへ
切り替えるにはまず文字を消す必要があった)。`docs/index.html`の
`<input type="text" id="model" list="model-list">` + `<datalist>`を
`<select id="model">`に置き換えた。

- **`<select>`は options に無い値を`.value`に代入しても入らない**(空文字に
  なる)。保存済みのモデル名を復元する`init()`と、設定同期の
  `mergeRemoteSettings()`は保存値をそのまま代入するため、必ず
  `renderModelOptions()`/`ensureModelOption()`を通すこと。ここを直接代入に
  戻すと、**モデル名が空のままAPIを呼ぶ**という分かりにくい壊れ方をする。
- `renderModelOptions(names, current)`は、`current`が`names`に無くても必ず
  候補に含める(一覧を取得していない端末でも保存済みのモデルが消えないため)。
- 一覧に無いモデル名は「直接入力…」(番兵`MODEL_CUSTOM_VALUE = '__custom__'`)
  から`prompt()`で追加する。取り消した場合は元の選択に戻すこと——番兵が
  選ばれたままだとその値がモデル名としてAPIへ送られる。
- **`MODEL_CUSTOM_VALUE`は`FILTER_STORAGE_PREFIX`と同じ理由でモジュール
  先頭側に置くこと**。`init()`はモジュール読み込み直後に即時実行され、その中の
  `renderModelOptions()`がこの値を参照するため、定義が後ろにあると`const`の
  TDZで`init()`がそこで止まりアプリ全体が中途半端な状態になる(実際にこの
  順序を誤って一度踏んだ。テスト[1]の「Worker URLの既定値が入っていること」
  が検知してくれた)。
- 「一覧を取得」の結果は`localStorage`(`STORAGE.modelList`)に保存するよう
  にした。以前はdatalistへ流し込むだけで、ページを開き直すたびに候補が空に
  戻っていた(毎回押し直す必要があった)。
- 回帰テストは`test_web_ui.mjs`の[1]。

#### 無料枠超過を「前払いクレジット切れ」と誤判定していた不具合(同時に修正)

上記のテストを書いている最中に判明した。`isBillingError`
(`docs/lib/gemini.js`)/`_is_billing_error`(`gemini_client.py`)が
**単なる `"billing"` という語だけで判定していた**が、Googleが無料枠の
上限超過で返す標準の文面にも `"please check your plan and billing details"`
が含まれる。そのため**ただの20回/日の超過なのに「前払いクレジットが
尽きている、新しいプロジェクトでキーを作り直してください」という、
そのとおりに操作しても解決しない案内**が表示されていた(2026-08-06に片桐が
実際に受け取ったのがこのメッセージ)。前払いクレジット切れの実際の文面は
`"Your prepayment credits are depleted."` なので、`prepayment` または
`credit`+`deplet` だけで十分に判定できる。`"billing"` 単独の条件を両実装から
削除した。回帰テストは`test_gemini.mjs`の[3]。

### 2026-08-05のバグ修正(まとめ)

片桐から「バグの検査や追加機能の提案を行ってほしい」との依頼を受けて全体を
...
**先に確認すること(2026-08-06追記)**: このメッセージが出ていても、実際には
**ただの無料枠の1日あたり上限**であることがある。2026-08-06まで
`isBillingError`/`_is_billing_error`が単なる`"billing"`という語だけで
判定しており、Googleが無料枠超過で返す標準の文面
(`"please check your plan and billing details"`)まで前払いクレジット切れと
誤判定していた(判定は修正済み)。エラー本文の`quotaId`に`PerDay`が含まれて
いれば無料枠の日次上限であり、**キーを作り直しても解決しない**
(太平洋時間の深夜まで待つか、上限の大きいモデルへ切り替える)。
...
## ローカルのTTSツール(既存apkgへの音声付け、2026-08-19追加)

「既存のapkgにTTS音声を付ける」機能をブラウザから使えるようにしたもの
(`local_tts_server.py` + `local_tts_page.html` + `apkgにTTS音声を付ける.bat`)。
Web版に無い唯一の機能がこれだったため、片桐の「PC版をこの機能だけに単純化し、
ブラウザから起動できるようにしたい」という提案を受けて着手した。

### 設計の経緯: なぜ「ローカルが画面ごと配信する」形なのか

当初は **GitHub Pagesのページから `http://127.0.0.1` を fetch する**
(ローカルツールをWeb版のバックエンドにする)設計で進めようとしたが、
**実測の結果それは成立しないことが分かった**。Chrome 151 を DevTools Protocol で
自動操作して本番ページ上で検証した結果:

| 条件 | 結果 |
| --- | --- |
| 通常のChrome(GitHub Pages → localhost) | ❌ `LocalNetworkAccessPermissionDenied` |
| ＋ `Browser.grantPermissions('localNetworkAccess')` / userGesture 付き | ❌ 変わらず |
| ＋ `--disable-features=LocalNetworkAccessChecks` | ✅ 成功 |
| **ローカルツール自身が配信したページから**(制限は有効のまま) | ✅ 成功 |
| ローカルページから外部API(Google TTS)へ | ✅ 到達(403=キー無しなので正常) |
| GitHub Pages から**リンク**でローカルへ遷移 | ✅ 成功 |

- リクエスト自体はサーバーに届いており(`Origin` も正しい)、CORSヘッダーも
  返っている。制限を切ると通ることから、**CORS・混在コンテンツ・localhostの
  扱いはいずれも問題ではない**。唯一の障壁が Chrome の Local Network Access
  権限で、プログラムからの権限付与も効かなかった。
- そのため、**画面もローカルツールが配信する**(同一オリジンにする)方式に変更した。
  Web版からは⚙設定の「既存のapkgにTTS音声を付ける(PCのみ)」にある**リンク**で
  繋ぐ(トップレベル遷移は制限を受けない)。
- **curlでは絶対に検証できない**(curlはCORSもLNAも無視する)。Workerの
  `Access-Control-Allow-Headers` に `Authorization` を入れ忘れて2週間気づかなかった
  のと同じ種類の落とし穴なので、この手の確認は必ず実ブラウザで行うこと。
  疎通確認用のサーバーは `tools/localhost_cors_check.py` に残してある。
- **副作用**: GitHub Pages とは別オリジンなので localStorage は共有されない。
  ただしこの機能はストック・Gemini・シートを一切使わないため実害は無い
  (TTSキーは PC 側の `config.json` から読む)。

### 実装(ローカルのTTSツール)

- **TTSのロジックは書かない**。`tts_core.analyze_targets` /
  `generate_tts_for_collection` / `export_collection` を呼ぶだけの層で、
  `tts_gui.py` と同じ約束(ロジックは `tts_core.py` に置く)を守ること。
- 待ち受けは `127.0.0.1` のみ。POSTは `Origin` を検査する。
  **APIキーはブラウザへ送らない**(`/api/settings` は `has_api_key` の真偽だけを返し、
  生成時は常に `config.json` の値を使う)。持ち出さなければ漏れようが無いため。
- エンドポイント: `GET /`(画面) / `GET /api/settings` / `POST /api/open`(apkgを
  そのままリクエストボディで受ける。`cgi` モジュールは Python 3.13 で削除済みなので
  multipart は使わない) / `POST /api/analyze`(ドライラン) / `POST /api/generate`
  (別スレッドで実行) / `GET /api/progress` / `POST /api/cancel` / `GET /api/download`。
- **2回目以降の読み込みでは、前回のCollectionを必ず閉じること**
  (`_prepare_work_col_path`)。`tts_core.load_collection` は作業ファイルを
  `os.remove` してから作り直すため、開いたままだとWindowsでは
  `WinError 32`(別のプロセスが使用中)になる。**実際に踏んだ**ので、
  閉じたうえで、それでも消せない場合は別名にフォールバックする。
- **成功・キャンセル・エラーのいずれでも、生成できた分は必ず書き出す**
  (`_run_generate`。PC版の「キャンセル時も途中状態を保持する」設計を、
  エラーで止まった場合にも広げたもの)。当初はエラーで例外が飛ぶと書き出しごと
  飛ばしていたため、**何百件生成した分がまるごと失われ、次回また同じ料金を
  払い直す**ことになる作りだった。生成済みのノートは既にcol側へ書き込まれて
  いるので、書き出しさえすれば無駄にならない。書き出し自体にも失敗した場合は、
  生成側のエラーを消さずに併記する(原因の切り分けができなくなるため)。
- **再開は「もう一度実行するだけ」**。`[sound:...]` が入ったフィールドは
  `analyze_targets` が「音声済み」として飛ばすため、続きから進む
  (ツールを終了してしまった場合も、書き出したapkgを①で読み込み直せば同じ)。
  実測: 8件生成済みのapkgを読み直すと「これから生成=0件 / 音声済みで飛ばす=8件」。
- **コンソール(黒い画面)を出さずに常駐する**。`apkgにTTS音声を付ける.bat` は
  `pythonw.exe` で起動する。**pythonwでは `sys.stdout` / `sys.stderr` が
  `None` になり、`print()` が `AttributeError` で落ちる**(`_log` がprintを使う)
  ため、`main()` の先頭で `local_tts_server.log` へ差し替えている。ここを
  外すと、生成のたびに例外で落ちるようになるので注意。
- **二重起動しても失敗しない**。`_already_running()` が先にポートを確認し、
  既に動いていれば画面だけ開いて終了する(バッチを何度押しても安全)。
- 終了はブラウザの画面の「ツールを終了する」(`POST /api/shutdown`)から。
  `serve_forever` を回しているスレッドから `shutdown()` を呼ぶと自分自身を
  待って固まるため、別スレッドから呼ぶこと。
- 終了時に `window.close()` で**タブも閉じる**(2026-08-19、片桐の要望)。
  ブラウザは原則これを拒否するが、HTML仕様の script-closable
  (そのタブが一度も他のページへ移動していない = `history.length === 1`)に
  当てはまれば閉じられる。ツールが自分で開いたタブはこれに当てはまる
  (実測で閉じることを確認済み)。当てはまらない場合に備え、400ms後に
  「手で閉じてください」と出す(閉じられた場合は誰も見ないので害はない)。
- **`.bat` の中身はASCIIのみにしてある**。cmdはcp932で読むため、UTF-8で
  日本語コメントを書くと文字化けする(`@echo off` なので表示はされないが、
  無用なリスクを避ける)。ファイル名の日本語は問題ない(Explorerからの
  ダブルクリックはUnicodeで解決される)。

### PC版(tts_gui.py)より良くした点

片桐のATSU用apkg(3ノートタイプ・英語フィールドが1ノートに8〜12個)を実際に
処理しようとすると、既存GUIでは次の3つで詰まることが分かったため、この画面では
最初から解消してある。

1. **既定の選択が危険だった**: `on_notetype_selected` は `Answer`/`Example` が
   無ければ**先頭フィールド**を選ぶ。ATSUのノートタイプの先頭は `DeckTitle`
   (日本語のデッキ名)なので、気づかず実行すると日本語を英語音声で読み上げる。
   新しい画面は「名前が `EN` で終わる」「`Answer`」「`Example`」のみを既定選択し、
   **該当が無ければ何も選ばない**。
2. **フィールド選択が1行ずつだった**: 3ノートタイプで合計30回の追加操作が必要
   だった。新しい画面は全フィールドをチェックリストで一覧表示し、
   「ENで終わるもの/すべて/解除」を一括で切り替えられる。
3. **ノートタイプごとに3回実行が必要だった**: `analyze_targets` は
   `note:"..."` で1ノートタイプずつ処理する。新しい画面は複数ノートタイプの
   指定をまとめて受け取り、1回の実行で順に処理する。

加えて、フィールドごとに**「中身のあるノート数」「既に音声がある件数」
「サンプル文字列」**を表示する。上記1のような取り違えは、サンプルが見えていれば
起こらない(`DeckTitle` の例に日本語が出る)。全ノートで空欄のフィールドは
チェックボックス自体を無効化する。

### 音声の再生ボタンが2つ並ぶ件(2026-08-19、片桐から報告)

生成したapkgをAnkiで見ると、一部のカードで**再生ボタンが2つ並ぶ**という報告。
不具合ではなく、`config.json` の `per_sentence_tags`(「文ごとにタグを分ける」)が
`true` になっているためで、この設定は**文の数だけmp3と`[sound:]`タグを作る**
(`tts_core.synthesize_per_sentence`)。

- 実データでの内訳: 英語フィールド475件のうち、**2文以上あるのは5件だけ**
  (例: `Pretty good. How are you?` → タグ2つ)。残り470件は1つ。
- 1つにまとめたい場合は「文ごとにタグを分ける」をOFFにする。`sentence_gap` の
  分だけ無音を挟んで1つの音声にする(`synthesize_with_gaps`)。この環境は
  `lameenc` が入っているのでMP3のまま出る(無ければWAVになり容量が増える)。
- **既に作った分を直すために全件を`force_regen`しないこと**。大半を占める
  1文だけのフィールドまで作り直すことになり、時間と割り当ての無駄になる。
  そのために `redo_multi_audio`(画面では「音声が2つ以上あるフィールドだけ
  作り直す」)を用意してある。`_build_plan` が force_regen で全件を取り直した
  あと、`[sound:` が2つ以上あるフィールドだけに絞る(実測で 475件 → 1件)。
- 同じ驚きを繰り返さないよう、**ドライランの時点で予告する**。
  「文ごとにタグを分ける」がONのとき、`_build_plan` が2文以上になる件数を
  数えて `multi_sentence` として返し、画面が警告色で表示する。
- ドライランと本番は**同じ `_build_plan` を使う**こと(以前は
  `analyze()` と `_run_generate()` が別々に `analyze_targets` を呼んでいた)。
  数えた件数と実際に処理される件数が食い違うと、この種の予告が信用できなくなる。

### 再生ボタンが2つ出る本当の原因: テンプレートの `{{tts}}`(2026-08-19)

片桐から「TTS音声を入れていない状態でもボタンがある(タグが無いのにボタンがある)」
との指摘で判明した。ATSUのノートタイプのテンプレートには
`<span class="tts-btn">{{tts en_US:I1BaseEN}}</span>` が**英語フィールドの数だけ**
入っている(実データで 12 + 10 + 8 = 30個)。

- `{{tts ...}}` は **Anki本体が端末の音声で読み上げる**ための標準タグで、
  フィールドに `[sound:...]` が無くてもボタンが出る。このツールが埋め込む
  Cloud TTS の音声と並ぶので、**全フィールドで再生ボタンが2つ**になる。
- **先に「文ごとにタグを分ける」が原因と説明したが、それは別件**(2文以上ある
  5件だけが対象)。全カードで2つ出ていた主因はこちら。両方が重なると3つになる。
- 片方が鳴らないように見えるのは、`{{tts}}` が端末側の音声エンジンを使うため
  (Windows版Ankiで en_US の音声が入っていないと無音になる)。
- 対処は `local_tts_server.strip_template_tts()`(画面の⑤「カードテンプレートの
  調整」)。**テンプレートだけを書き換え、ノートの中身と音声には触らない**。
  `<span class="tts-btn">…</span>` の包みごと消すので空のspanも残らない。
  実測: 30個 → 0個、ノート77件はそのまま。
- **元のapkgを作り直す必要はない**。作り直すと `[sound:]` が消えて475件の
  TTSを再生成することになる(時間と割り当ての無駄)。生成済みのapkgを
  読み込んで⑤を実行すれば、音声はそのままでボタンだけ1つになる。
- ダウンロードは `STATE.last_output` を見る。TTS生成と⑤のどちらの後でも
  同じ「.apkgをダウンロード」で取れるようにするため。
- **⑤の単独実行だけでは順番を間違える**(2026-08-19に実際に起きた)。⑤は
  「いま読み込んでいるコレクション」に効くので、その後にapkgを読み直して
  生成すると元のテンプレートのまま書き出される。そのため③に
  「Anki標準の再生ボタン({{tts}})も取り除く」(`strip_template_tts`)を用意し、
  **書き出しの直前**に適用するようにした。`{{tts}}` を含むapkgを読み込んだ
  ときは既定でONになる。⑤は「もう生成し終わったapkgを直す」用に残してある。
- **Anki側は取り込みでノートタイプを更新しないことがある**。同じmodel_idの
  ノートタイプが既にあると、テンプレートが古いまま残りうる(片桐の環境で、
  取り込み後もテンプレートに `{{tts}}` が残っていた)。その場合は Anki の
  「ノートタイプを管理 → カード」で表面テンプレートを貼り替えるのが確実
  (貼り替えれば全カードに即座に反映される)。

### 同じ名前で上書きしたapkgが読み込まれない(2026-08-19、片桐から報告)

「apkgを作り直して同じファイルに上書きし、①で選び直したのに『対象がありません』
になる」という報告。原因は**ブラウザが同じパスの再選択では`change`を発火しない**
こと。新しい中身が一切読み込まれず、**前回の生成済みコレクションが残ったまま**
なので「全フィールドが音声済み=対象0件」になる。

- 対処は `input.value = ''`(`local_tts_page.html` の `#file` のハンドラ冒頭)。
  **Web版(`docs/app.js` のバックアップ読み込み)では既に対処済みで、CLAUDE.mdにも
  書いてあった罠を、このローカル画面に入れ忘れていた**。新しく
  `input[type=file]` を足すときは必ず同じ対処を入れること。
- 気づけるように、読み込み結果の表示に**「既に音声があるフィールド N件」と
  読み込み時刻**を出すようにした。読み込み直せていれば0件になるので、
  古いまま処理を続けていないかが一目で分かる。
- 「対象が0件です」のメッセージにも、音声済みで飛ばした件数と
  「①で選び直す/『既存の音声を作り直す』をONにする」の案内を入れた。
- 回帰テスト: `scratchpad/cdp_reload_test.py` 相当(実ブラウザで同じパスの
  中身を差し替えて選び直し、読み直せることを確認)。実測で
  「音声0件 → 7件」と表示が更新されることを確認済み。

### Web版からPC上のツールを起動する(カスタムURLスキーム、2026-08-19追加)

片桐から「Web版の画面からこのBATを実行できるボタンが欲しい」との要望。
**Webページから直接プログラムを起動する手段はブラウザに存在しない**
(あれば、どのサイトも勝手にPCのプログラムを実行できてしまう)。唯一の正規の
方法がカスタムURLスキームで、Zoomの `zoommtg:` やSlackの `slack:` と同じ仕組み。

- `tools/register_local_tts_protocol.py` が
  `HKCU\Software\Classes\ankitts` に「このURLを開いたら pythonw で
  `local_tts_server.py` を起動する」と登録する(**管理者権限は不要**。
  取り消しは `--unregister`、確認は `--show`)。
- Web版に `<a href="ankitts://start">` を置く。押すとツールが起動し、
  ツール自身がブラウザでローカル画面を開く。**置き場所はヘッダー**
  (`header-launch-tts`)。最初は⚙設定の中だけに置いたが、設定の下の方に
  埋もれて「ボタンが無い」と報告されたため表に出した(⚙設定の中にも、
  登録方法の説明と一緒に残してある)。スマホでは動かないので、
  `isMobileBrowser()` が偽のときだけ `hidden` を外す。
  回帰テストは `test_web_ui.mjs` の[1]。
- **PCごとに1回だけ登録が必要**。登録していないPC・スマホでは押しても
  何も起きない(その旨を画面の説明に明記してある)。
- ブラウザは初回に「このサイトは ankitts を開こうとしています」と確認する。
  これはブラウザ側の安全機構でありこちらからは消せない(「常に許可」で次回から出ない)。
- 実測(2026-08-19): サーバー0プロセスの状態から `ankitts://start` を開くと
  1プロセス起動し、`http://127.0.0.1:8765/` が200を返すことを確認。
  `%1` として渡る `ankitts://start/` は `local_tts_server.py` が無視する。

### 検証状況(2026-08-19)

`ATSU_日常英会話完全マスター講座_音読用_TTS対応.apkg`(77ノート)で確認済み。
UI操作もCDPで自動化して通した(ファイル選択は `DOM.setFileInputFiles`)。

- 読み込み → 3ノートタイプを検出、EN終わりの28フィールドを自動選択、
  `DeckTitle` は選ばれない、空欄フィールド2つは無効化
- ドライラン → **475件 / 10,048文字**(独立に数えた値と一致)
- 実生成 → 最小範囲(8件)だけ実行し、出力apkgを検証:
  `collection.anki21` に77ノート・うち8件に `[sound:...]`、
  メディア8件が実体のあるMP3、タグは対象フィールドの末尾に `<br>` 付きで追記。
  **`collection.anki2` の方は1ノートしか無いが、これは `legacy=True` 出力に
  含まれる旧クライアント向けのプレースホルダで、正常**(検証時にこちらを見て
  「ノートが1件しかない」と誤読しかけた)。
- **未実施**: 475件の本番実行(実行すればAPI呼び出しが475回発生する)。
  片桐が毎分の割り当てを下げているため、そこが律速になる見込み。
- **未着手**: PC版(`tts_gui.py`、4,604行)をこの機能だけに単純化する作業。
  **削るのはGUIであって、モジュールではない**点に注意 —— `card_defs.py` /
  `card_def_builder.py` / `build_*.py` は `tools/export_shared_card_defs.py` と
  `npm run verify` が依存しており、消すとWeb版のカード定義が生成できなくなる。
  ⚙設定の「カード定義」タブを消す場合は、Web版のカード定義を編集する手段を
  先に用意すること。
...
docs/
  ...
  lib/sync.js                       複数端末間の同期のマージロジック(id単位の
                                     ...
                                     「Web版の複数端末間の同期」を参照)
  lib/quota.js                      Gemini APIの呼び出し回数カウンタ。上限値は
                                     持たず、429応答から学習する(2026-08-06追加。
                                     詳細は「Gemini APIの使用状況カウンタ /
                                     まとめて生成(バッチ)」を参照)
...
tools/
  ...
  test_quota.mjs                    lib/quota.js(呼び出し回数の記録・429からの
                                     上限の学習)の単体テスト
                                     (npm run test:quota、2026-08-06追加)
...
- **`docs/`配下を変更したら必ず`cd tools && npm test`を通すこと**
  (初回のみ`npm install`。`tools/node_modules/`はGit管理外)。中身は9本
  (2026-08-06時点で387アサーション)。
  **2026-08-05にGitHub Actions(`.github/workflows/test.yml`)を追加した**ので、
  push/PRのたびに自動でも回る(手元での実行が不要になったわけではない。
  失敗に気づくのが早いほうがよいため、手元でも通してからpushすること)。
  外部サービスには一切アクセスしないので、CI側にシークレットの設定は不要。
  - **`card_defs.json`はGit管理外**(実データを含みうるため)。クリーンな
    チェックアウトでは存在せず、そのままだと`verify`が
    「カード定義が見つかりません」で落ちる。そのため
    `tools/dump_python_apkg.py`が、デスクトップ版の起動時と同じ
    `card_defs.seed_default_*_if_missing()`を自前で呼ぶようにしてある
  ...
  - `npm run test:ui`(`test_web_ui.mjs`): jsdom上で`index.html`+`app.js`を
    ...
    **Gemini APIはfetchをモックするのでAPIキー・割り当てを消費しない**
    ...
    2026-08-05に[24]バックアップの書き出し/読み込み(特に**削除済みの項目も
    復元できること**)と、[25]起動時の自動読み込みの設定の永続化を追加した。
    **`geminiMode = 'grammar_multi'`で生成するセクションを足す場合は、
    直前に`geminiCalls = 0`を入れること**——このモックは「1回目=3問、
    2回目以降=習熟用4問目」を累積カウンタで判別しているため、リセットを
    忘れるといきなり4問目の応答が返り`parsed.map is not a function`になる
    (実際に一度踏んだ)。
  ...
  - `npm run test:sheets`(`test_sheets.mjs`、2026-07-29追加):
    ...
    症状になるため。
    2026-08-05に**401時の自動リトライの検証[7]を追加**した(Worker方式では
    401→`/refresh`→新しいトークンで再送まで自動で通ること、リトライは1回
    だけで打ち切ること、従来方式(B)では自動リトライしないこと)。
    **(B)の検証で`getAccessToken({workerUrl: ''})`を素で呼ぶとテストが
    止まる**——GISのスクリプト読み込み(`loadGisScript`)がjsdomでは永久に
    解決しないため。先に有効なアクセストークンを持たせてから呼ぶと、
    `hasValidAccessToken()`の早期returnでGISに触れずに`lastAuthOptions`だけ
    切り替えられる(テスト内のコメント参照)。
  ...
  - `npm run test:worker`(`test_worker.mjs`、2026-08-05追加): `worker/src/index.js`
    ...
    **client_secretを一切返さないこと**・`/token`が`client_secret`を
    ...
    2026-08-05に`GET /appconfig`の検証[5]を追加した。**秘密情報を返す
    エンドポイントなので、本人確認を通さずに返してしまわないことが最重要**:
    トークン無し/別アカウント/別アプリ向け/期限切れ/メール未確認をすべて
    拒否し、かつ**`ALLOWED_EMAIL`未設定なら誰にも返さない(fail closed)**
    ことを固定してある。
  - `npm run test:quota`(`test_quota.mjs`、2026-08-06追加): `lib/quota.js`の
    単体テスト。**上限値をソースに持たない**設計の要点を固定してある
    (429応答の`quotaValue`からの学習、日次(PerDay)だけを学習し分あたり
    (PerMinute)は学習しないこと、日付が変わってもカウントは0に戻すが学習した
    上限は忘れないこと、リトライも1回として数えること)。最小限の
    `localStorage`を用意してから`gemini.js`を読み込むので、
    `callGemini()`経由での記録・学習まで通しで検証できる。
    `localStorage`が使えない環境で例外にならないことも確認している。
  - **`verify`系2本が呼ぶPythonは環境変数`ANKI_TOOL_PYTHON`で差し替えられる**
    (2026-08-29追加。既定は従来どおり`python3`)。この2本は
    `execFileSync('python3', ...)`とハードコードされており、`python3`という
    名前で起動できるPythonが無い環境ではここだけ失敗していた。片桐のWindows
    実機では**WindowsAppsのスタブ**が先に見つかり、genankiの入った
    `C:\Python314\python.exe`とは別物になる(`exit status 9009`)。

        ANKI_TOOL_PYTHON=/c/Python314/python.exe npm test

    以前は「この2ファイルの`'python3'`を実際に使えるコマンドに直すこと」と
    書いていたが、それだと毎回ソースを書き換えることになり、その状態で
    コミットしてしまう事故のもとだったため環境変数に変えた。
- 実装時に判明した注意点:
...
#### 同期データの分割保存(2026-08-05追加)

片桐から「**AIに質問の占有率が50%近く、すぐ100%になってしまう**」と報告を
受けての対応。実データで測ると Grammar Multi は**1件あたり約1,020文字**
(内訳の大半は`<div class="whynot-item">`等のHTML定型部分)で、1セル
50,000文字では**約49件 = 質問16回分**しか入らなかった。

- **Sheetsの50,000文字制限は「1セルあたり」で、シート全体ではない**
  (1シートは1,000万セルまで持てる)。そこで1キーの値を複数セルへ分けて書き、
  読むときに連結する。A列がキー名、**B列以降がチャンク**という構成。
  `sync.js`の`splitIntoChunks`/`joinChunks`、`sheets.js`の
  `readSyncState`/`writeSyncState`が担当する。
- `SYNC_CHUNK_SIZE = 45000`(50,000ちょうどにしないのは、Googleが数える
  「文字数」とJavaScriptの`String.length`(UTF-16のコード単位数)が絵文字などの
  BMP外文字でずれるため)、`SYNC_MAX_CHUNKS = 20` → **1キーあたり
  900,000文字 ≒ Grammar Multi で約880件(質問290回分)**。
  **増やす副作用はほとんど無い**(使わないセルは空文字で書かれるだけ)ので、
  逼迫したら`SYNC_MAX_CHUNKS`を増やせばよい。
- **下位互換**: 読み取りは「B列以降を全部連結する」だけなので、分割対応より
  前に書かれたデータ(1キー=1セル)もそのまま読める。片桐の既存データに
  移行作業は不要(`test_sync.mjs`の[4.5]で固定してある)。
- **使わないチャンク列も必ず空文字で埋めること**。短い行を書くとSheetsは
  足りない分を「変更なし」として扱うため、前回より短くなったときに古い
  チャンクが残り、読み直したとき壊れたJSONになる。
- 上限を超えた場合は**切り詰めずに例外を投げる**(切り詰めると読み直した
  ときにJSONとして壊れるため)。通常は`app.js`の`runSync`が
  `exceedsSyncLimit`で事前に止める。
- `capacityPercent`の分母は1セル(50,000)ではなく`SYNC_VALUE_LIMIT`(900,000)に
  変更した。`exceedsCellLimit`は`exceedsSyncLimit`へ改名してある。
- **圧縮は見送った**(2026-08-05)。実データでgzip+base64を測ると約3.7分の1に
  なる(HTMLの定型部分が繰り返されるためよく縮む)が、分割保存で既に20倍の
  余裕があり、単体では16回→52回にしかならない。中身がシート上で読めなく
  なる・`CompressionStream`非対応環境へのフォールバックが要る、という
  デメリットに見合わないと判断した。将来さらに逼迫した場合は、まず
  `SYNC_MAX_CHUNKS`を増やすこと。それでも足りなければ圧縮を検討する。
...
**2026-08-05に、書き込み前の上限チェックと事前警告を追加した**。それまで
`capacityPercent`は計算していたものの、表示するのは`writeSyncState`が
**成功した後**だった。使用率が100%を超えた瞬間、Sheets APIが素の400を返して
同期が丸ごと止まり、しかもどのストックが原因か分からないメッセージになる
(「AIに質問が19.6%」という報告が既に出ており、到達しうる状態だった)。

- `exceedsCellLimit(jsonString)`: 上限超過の判定。`runSync`が`writeSyncState`の
  **前**に全ストックを検査し、超えていたら書き戻しを中止して、原因のストック名と
  対処方法(各タブの「出力済みを削除」)を添えたメッセージを出す。
  **このときリモートの取り込み(マージ)自体は既に完了している**ので、その旨も
  明記する(データが失われたと誤解されないため)。
- `CAPACITY_WARN_PERCENT`(70%): この使用率を超えたら、まだ書き込めるうちに
  警告する。上限に達してからでは「同期が一切通らない」うえに、復旧手段を
  実行するために開くアプリ自体が同期できないという手詰まりになりやすいため、
  余裕のあるうちに知らせる設計にした。

#### 実装(複数端末間の同期)

- `docs/lib/sync.js`(新設): `mergeStock`/`ensureItemIds`/`capacityPercent`/
  `parseIdArray`/`newSyncId`/`exceedsCellLimit`。通信を一切持たない純粋関数
  のみで、`tools/test_sync.mjs`が単体テストする。
- `docs/lib/sheets.js`: `readSyncState`/`writeSyncState`(+ 内部の
  ...
  小さくするため)。
  - **`readSyncState`はA列のキー名で引くこと**(2026-08-05修正)。以前は
    `values[i][1]`と**行の位置だけ**で読んでおり、A列に書いてあるキー名を
    一切照合していなかった。将来`SYNC_ROW_KEYS`の順序が変わる・途中にキーが
    増えると、既にシートを持っている端末が「単語のJSONを習熟用として
    読み込む」取り違えを起こす——エラーにならず静かにストックが混ざり、
    次の書き戻しで他端末にも伝播するため気づいたときには手遅れになる。
    キーで引いておけば順序の変更に対して無害になる。
...
- **ログイン/同期の状態表示は「ページ上端の通知バナー」、ログ本体は⚙設定
  (2026-08-06、置き場所の4度目にして最後の変更)**: この状態表示
  (`header-auth-status`/`header-sync-status`)は、ヘッダーの枠の中の文字 →
  「🗒 ログ」ボタン直下の`position:absolute`ポップアップ → 画面右上の
  `position:fixed`トースト、と短期間に3度置き場所を変えたが、いずれも
  「ヘッダーのボタンに被って読みづらい」という問題が残った。**画面に
  浮かせる方式である限り、下にある何かは必ず隠れる**というのが結論。
  現在はページの一番上に横いっぱいで差し込まれるバナー(`#notice-banner`、
  `position: sticky; top: 0`)にしてある。stickyは通常のレイアウトに参加
  するため、表示された分だけ下の内容(ヘッダー・タブ)が押し下げられて
  **何とも重ならない**うえ、スクロールしても画面上端に残る。
  中身の`<p>`が全て`hidden`(=`display:none`)のときは高さ0になるので、
  何も出ていない間はページの見た目に一切影響しない(そのため余白は土台側の
  padding/gapではなく、個々の`.notice`のmarginで付けている)。
  - **同時に「🗒 ログ」ボタンを廃止し、ログ本体を⚙設定の「ログ」節へ
    移した**(`#app-log`、`appendAppLog()`)。旧ボタンは「自動で隠れた
    直前の1件をもう一度出す」だけのもので、履歴を追えなかった。現在は
    `data-log`属性を持つ状態表示の文言を`setStatus`が履歴として積む
    (最大`APP_LOG_MAX`=50件、新しいものが上、保存はせずページ再読み込みで
    消える)。`showLoading`の経過表示(「〜中...」)は積まない——結果は必ず
    最後の`setStatus`で記録されるため、積むとログが読みにくくなるだけ。
  - **ヘッダーの枠(`.header-actions`)からoverflow指定をまるごと撤去した**。
    「枠の高さを一定に保つ」ため`flex-wrap: nowrap` + `overflow-x: auto`で
    横スクロールに逃がしていたが、狭い画面でボタンが見切れ、スクロール
    しないと全部を見られない状態になっていた(片桐の指摘)。加えて、
    **CSSの仕様上`overflow-x`だけを指定すると`overflow-y`も`auto`扱いに
    なる**ため、`overflow-y: visible`を明示しても⋮メニュー(下へ飛び出す
    `position:absolute`)のクリップを防げていなかった。素直に折り返す方式に
    変更し、`.toolbar-group`も`flex-shrink: 1`にした(枠ごと画面外へ
    はみ出してページが横スクロールするのを防ぐため)。ログボタンが減った
    ことで、通常の画面幅ではそもそも折り返さない。
  - `.status[hidden] { display: none; }`をCSSに明示してある。`.status.loading`が
    `display:flex`を直接指定しているため、これが無いと`hidden`属性が効かない
    (`.header-menu-panel[hidden]`と同じパターン)。目立つ位置にあるバナーで
    「消えるべきものが消えない」は分かりやすい不具合になるので念のため。
  - **この変更の直後に実機で「⋮メニューが開かない」「OAuthクライアントID
    またはWorker URLを設定してください、と出てログインできない」という
    2つの症状が同時に出た(2026-08-06)**。原因は**ブラウザが古い`app.js`を
    キャッシュしたまま新しい`index.html`を読み込んだこと**(この変更で
    `header-status-reveal`を削除し`app-log-clear`を追加したため、
    どちらが古くてもIDが食い違う)。`bindEvents()`が
    `$('id').addEventListener(...)`を素で並べていたため、**1つでもIDが
    見つからないとそこで例外になり、`bindEvents()`の残り全部と、それを
    先頭で呼ぶ`init()`の残り全部(Worker URLの既定値の設定など)が丸ごと
    実行されない**という壊れ方をしていた——症状が2つに見えたのはこのため
    (⋮のイベント登録が飛ぶ + Worker URLが空のまま)。対策は2つ:
    - `on(id, type, handler)`ヘルパーを追加し、`bindEvents()`内の登録を
      すべてこれ経由にした(要素が無ければ警告を出して**その1件だけ**
      飛ばす)。`bindPersistentCheckbox`も同様。多少の食い違いでアプリ
      全体が使えなくなることはもう無い。
    ...
    - **実際に3度目の再発(2026-08-20)**。習熟用のフィールド構成をv2へ
      変えたとき、`docs/`配下を変更したのに`?v=`を更新し忘れ、ブラウザが
      古い`app.js`と`lib/shuujuku.js`を読み続けて**旧Num/Content形式の
      カードが出力され続けた**。対策として`?v=`の適用範囲を広げた:
      - `lib/shuujuku.js`の`import`にも`?v=`を付けた(`lib/sheets.js`と
        同じ方式。**他から`import`されていないモジュールにだけ付けること**
        ——同じモジュールを別々のURLで読むと実体が2つできる)。
      - `shared/`配下(プロンプト・`card_defs.json`・`anki_schema.json`)は
        `fetchText()`が`APP_VERSION`を付けて取りに行くようにした。
        `card_defs.json`が古いまま新しい`lib/*.js`と組み合わさると、
        フィールドが空のカードが出力されてしまうため。版は`app.js`自身の
        `?v=`から取るので、`index.html`の値を更新すれば一緒に切り替わる
        (開発時=`?v=`無しの場合は`(開発版)`になるので付けない)。
    - 回帰テスト: `test_web_ui.mjs`の[1]に「Worker URLの既定値が入って
      いること」(=`init()`が最後まで進んだ目印)を追加した。
- **ヘッダーの⋮メニューを廃止し、「⚙ 設定」を直接置いた(2026-08-06)**:
  2026-07-30に「ログイン周りがごちゃごちゃしている」との指摘を受けて
  設定を格納するために作ったメニューだが、状態表示とログをこの枠から
  追い出してボタンが減り幅に余裕ができたため、片桐の指示で表に戻した
  (中身が「設定」1つだけのメニューは、ワンクッション増やすだけで
  意味が無くなっていた)。`openHeaderMenu`/`closeHeaderMenu`/
  `onHeaderMenuToggle`と、外側クリック・Escapeで閉じる`document`
  レベルのリスナー、`.header-menu`/`.header-menu-panel`のCSSは
  まるごと撤去済み。
...
#### 起動時の自動読み込み(2026-08-05追加)

同期は手動ボタンのみだったため、押し忘れると「古い状態の端末で作業を始めて
しまう」という、衝突のいちばんの原因が残っていた。ページを開いたときに
シートの内容を**読み込むだけ**の同期を自動で行うようにした
(`autoPullOnStartup()`、`docs/app.js`)。

- **書き戻し(push)は自動化しない**。利用者が何も操作していないのにこの端末の
  内容でシートを上書きすることになり、意図しない伝播(例: 別端末で追加された
  直後に、古い端末を開いただけで巻き戻る)が起きうるため。マージ自体は
  和集合なので pull だけでもデータは失われない。
- `runSync`(pull-merge-push)との共通部分は`mergeRemoteIntoLocal(remote)`に
  切り出してある。戻り値の`newState`は`writeSyncState`にそのまま渡せる形で、
  pullのみの経路では使わないだけ。`added`(増えた件数)は「何件取り込んだか」の
  表示に使う。
- **非ブロッキング**: 未ログイン・設定未入力・通信失敗のいずれでも静かに
  諦めてアプリの起動を続ける。`init()`からは**awaitせずに**呼ぶ(通信を待つ間
  アプリが使えないと不便なため)。取り込みが0件のときは何も表示しない
  (毎回の起動でメッセージが出るとノイズになる)。
- **起動時に勝手にログインさせない**: `isSignedIn()`が偽なら何もしない。
  でないと、ページを開いただけで同意画面のポップアップやページ遷移が
  起きてしまう。
- ⚙設定「複数端末間の同期」の「起動時に自動で読み込む」チェックボックス
  (`sync-auto-pull`、既定ON)で切れる。`bindPersistentCheckbox`で永続化して
  おり、**`bindEvents()`内の登録は`init()`末尾の`autoPullOnStartup()`より
  前に行われる必要がある**(復元した値を見て判断するため。`bindEvents()`は
  `init()`の先頭で呼ばれるので現状は問題ない)。

#### バックアップ(書き出し / 読み込み、2026-08-05追加)

デスクトップ版はapkg生成のたびに`backup/`へ自動保存するが、Web版は
`downloadBlob()`がブラウザのダウンロード機構を呼ぶだけで、アプリ側は生成物を
一切保持していない(File System Access APIによる保存先固定はモバイルでの
対応状況が不安定なため見送られている)。つまり**ストックの項目こそが「Ankiに
取り込む前の内容」を再現できる唯一のコピー**であり、「出力済みを削除」や
ブラウザのデータ削除で失うと復元できない。この非対称性を埋めるための、
依存を増やさない最小の手当て(⚙設定の「バックアップ」節)。

- **複数端末間の同期とは役割が違う**。同期はシート上に1セット(=最新の状態)
  しか持たないため、誤って削除した内容は同期しても戻ってこない(むしろ削除が
  他端末へ伝播する)。バックアップは「その時点のスナップショットをファイルと
  して手元に残す」もの。
- 書き出し(`onBackupExport`): 3ストックの内容 + 習熟用の続き番号
  (`shuujuku_next_num`)を1つのJSONにして`downloadBlob`で保存する。
  `format: 'anki-tool-backup'`と`version`を持たせてあり、読み込み側が
  無関係なJSONを弾ける。
- 読み込み(`onBackupFileSelected`): **現在の内容に足し合わせる**
  (既存項目は消さない)。**削除済み(打ち消し記録にあるid)の項目は、
  新しいidを振り直して復元する**——これが実装上の要点で、元のidのまま
  マージすると`mergeStock`が打ち消し記録を見て除外し、「バックアップから
  戻したのに復活しない」という一番困る挙動になる。打ち消し記録から
  取り除く方式は、リモート側の記録が残っている限り次の同期でまた消える
  ため解決にならない。新しいidにすれば他端末へも普通の新規追加として
  伝わる。同じ内容が二重に見えることはあるが、このアプリは元々
  「重複は常に追加して⚠表示し、手動で間引く」方針で統一されており、
  既存の重複検出UIでそのまま解消できる。
- 習熟用の続き番号は**既存の値とバックアップの値の大きい方**を採用する
  (小さい方に戻すと、既にAnkiにあるカードとNumが衝突しうるため)。
- `input[type=file]`は同じ値の再選択では`change`が起きないため、
  ハンドラの先頭で必ず`input.value = ''`しておくこと。

#### 打ち消し記録(tombstone)の刈り込み(2026-08-05追加)

削除のたびにidが1つ増え、**無期限に増え続ける**設計だった。tombstoneは
itemsと同じ50,000文字のセル上限を持つ別の行に入るため、UUID(36文字)+
引用符・カンマで1件約39バイトとして1,280件ほどで書き込めなくなる。

- `sync.js`の`pruneTombstoneIds(ids, max = MAX_TOMBSTONES)`が上限
  (`MAX_TOMBSTONES = 500`、約20KB = セル上限の約39%)を超えたぶんを
  **古い方から**捨てる。`mergeStock`の戻り値と、`app.js`の
  `saveTombstoneIds`の両方を通す。
- **「古い」は配列の並び順で判断している**。記録はid文字列だけで時刻を
  持たない(時刻を持たせると1件あたりの容量が倍近くなり、容量を減らすという
  目的と衝突する)。そのため**ソートしてはいけない**——2026-08-05に
  `mergeStock`と`saveTombstoneIds`の両方から`.sort()`を外した。ソートすると
  並びがUUIDの辞書順になり、刈り込みの判断材料としての意味を失う。
  `Set`は挿入順を保つので、重複除去だけなら順序は壊れない。
- **捨てても安全な理由**: 打ち消し記録は「古いスナップショットを持つ端末が
  同期しても復活させるな」という目印であり、全端末が同期を終えた後は不要に
  なる。問題が起きるのは「非常に古い削除を、まだ一度も同期していない端末が
  持っている」場合だけ。万一復活しても**データ消失ではなく一覧に重複が
  現れるだけ**で、既存の「⚠ 重複」表示から手動で消せる(このアプリ全体の
  設計方針と同じ)。
...
- クライアントID・スプレッドシートIDは(APIキーと同様に)意図的に**複数端末間の
  同期の対象外**にしてある(実データではなく秘密情報に準ずる設定値のため、
  「Web版の複数端末間の同期」の項を参照)。そのため、PCで設定していても
  スマホ側では別途入力しない限り空のまま。
- スプレッドシートIDの入力欄にはプレースホルダーとして
  `1ruIEHQk8J0rjjQQdiaT2Nvnje...`という(実物そっくりの)灰色の例文が
...
- **対応**: `runSync`がこのエラーになった際、⚙設定を自動的に開き、
  ...
  「次に同じ状況になったときに自己診断しやすくする」ための改善。
  エラー文言にも「これらの値はブラウザごとに別々に保存される」ことを
  明記した。
- 引き続き実機での再現確認待ち。もしフィールドが実際に空だった場合は
  単純な未設定(スマホ側で改めて入力すれば解決)、濃い文字で値が入って
  いるのにこのエラーが出る場合は追加調査が必要(その場合はブラウザの種類・
  「ホーム画面に追加」で開いているか等の情報が手がかりになる)。
