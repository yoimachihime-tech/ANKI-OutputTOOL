...
const {
  mergeStock, ensureItemIds, capacityPercent, parseIdArray, newSyncId,
  SHEET_CELL_LIMIT, SYNC_VALUE_LIMIT, SYNC_CHUNK_SIZE, SYNC_MAX_CHUNKS,
  exceedsSyncLimit, splitIntoChunks, joinChunks,
...
{
  const json = 'a'.repeat(SYNC_VALUE_LIMIT / 2);
...

{
  ...
  // A列=キー名 + B列以降=チャンク(SYNC_MAX_CHUNKS個) なので右端は colLetter(20)='U'
  if (put && put.url.includes(encodeURIComponent(`${SYNC_SHEET_NAME}!A1:U${SYNC_ROW_KEYS.length}`))
    && put.url.includes('valueInputOption=RAW')) {
    ok('A列(キー名)+チャンク列ぶんのレンジに valueInputOption=RAW で書き込む');
  } else {
    fail(`書き込みリクエストが想定と違う: ${put ? put.url : '(PUTが呼ばれていない)'}`);
  }

  const row0 = put.body.values[0];
  if (row0[0] === 'word_stock_items' && row0[1] === '[{"id":"a"}]') {
    ok('各行はSYNC_ROW_KEYSの順に [キー名, チャンク...] を書く');
  } else {
    fail(`書き込む値の形が想定と違う: ${JSON.stringify(row0.slice(0, 3))}`);
  }

  // **使わないセルも空文字で埋めること**。短い行を書くとSheetsは足りない分を
  // 「変更なし」として扱うため、前回より短くなったときに古いチャンクが残り、
  // 読み直したとき壊れたJSONになる。
  if (row0.length === SYNC_MAX_CHUNKS + 1 && row0.slice(2).every((c) => c === '')) {
    ok('使わないチャンク列も空文字で埋める(前回の残骸を消すため)');
  } else {
    fail(`行の長さ/埋め方が想定と違う: length=${row0.length}`);
  }
  ...
  if (emptyRow[1] === '') {
    ...
    fail(`未指定キーの扱いが想定と違う: ${JSON.stringify(emptyRow.slice(0, 3))}`);
  }
}

// ---------------------------------------------------------------------------
// [4.5] 1セルの上限を超える値の分割保存(2026-08-05追加)
//
// 片桐から「AIに質問の占有率が50%近く、すぐ100%になってしまう」と報告を
// 受けての対応。Sheetsの50,000文字制限は**1セルあたり**でシート全体では
// ないため、値を複数セルへ分けて書き、読むときに連結する。
// ---------------------------------------------------------------------------
console.log('\n[4.5] 1セルを超える同期データの分割保存');

{
  // 3セル分に収まる長さ(チャンク境界をまたぐことを確認したいので端数を付ける)
  const big = 'x'.repeat(SYNC_CHUNK_SIZE * 2 + 123);
  const calls = mockFetch(async (url) => {
    if (url.includes('fields=sheets.properties.title')) {
      return { sheets: [{ properties: { title: SYNC_SHEET_NAME } }] };
    }
  ...
  await writeSyncState({
    spreadsheetId: 'SHEET_ID', accessToken: TOKEN, state: { ai_ask_stock_items: big },
  });

  const put = calls.find((c) => c.method === 'PUT');
  const row = put.body.values[SYNC_ROW_KEYS.indexOf('ai_ask_stock_items')];
  if (row[1].length === SYNC_CHUNK_SIZE && row[2].length === SYNC_CHUNK_SIZE
      && row[3].length === 123 && row[4] === '') {
    ok('1セルに収まらない値はチャンクに分けて複数セルへ書く');
  } else {
    fail(`分割の仕方が想定と違う: ${row.slice(1, 5).map((c) => c.length)}`);
  }
  if (joinChunks(row.slice(1)) === big) {
    ok('分割したチャンクを連結すると元の値に戻る');
  } else {
    fail('連結しても元の値に戻らない');
  }
}

{
  ...
  mockFetch(async (url) => {
    ...
    return { values: [['ai_ask_stock_items', '["ab', 'c","de', 'f"]']] };
  });
  const state = await readSyncState({ spreadsheetId: 'SHEET_ID', accessToken: TOKEN });
  if (state.ai_ask_stock_items === '["abc","def"]') {
    ok('読み取り時はB列以降を連結して1つの値に戻す');
  } else {
    fail(`連結結果が想定と違う: ${state.ai_ask_stock_items}`);
  }
}

{
  // **下位互換**: 分割対応より前に書かれたデータ(1キー=1セル)もそのまま読める。
  // これが壊れると、片桐の既存データが読めなくなる。
  mockFetch(async (url) => {
    if (url.includes('fields=sheets.properties.title')) {
      return { sheets: [{ properties: { title: SYNC_SHEET_NAME } }] };
    }
    return { values: [['word_stock_items', '[{"id":"old"}]']] };
  });
  const state = await readSyncState({ spreadsheetId: 'SHEET_ID', accessToken: TOKEN });
  if (state.word_stock_items === '[{"id":"old"}]') {
    ok('分割対応より前の1セルだけのデータもそのまま読める(移行作業が不要)');
  } else {
    fail(`下位互換が壊れている: ${state.word_stock_items}`);
  }
}

{
  // 上限を超える値は、切り詰めて壊れたJSONを書くのではなく明示的に失敗させる
  const huge = 'x'.repeat(SYNC_CHUNK_SIZE * SYNC_MAX_CHUNKS + 1);
  mockFetch(async (url) => {
    if (url.includes('fields=sheets.properties.title')) {
      return { sheets: [{ properties: { title: SYNC_SHEET_NAME } }] };
    }
  ...
  try {
    await writeSyncState({
      spreadsheetId: 'SHEET_ID', accessToken: TOKEN, state: { ai_ask_stock_items: huge },
    });
  } catch (e) { thrown = e; }
  if (thrown && /上限を超え/.test(thrown.message)) {
    ok('上限を超える値は切り詰めず、対処方法を添えて失敗させる');
  } else {
    fail(`上限超過の扱いが想定と違う: ${thrown?.message}`);
  }
}

{
  // splitIntoChunks / joinChunks の素の挙動
  if (deepEq(splitIntoChunks(''), []) && deepEq(splitIntoChunks(null), [])) {
    ok('splitIntoChunks: 空の値はどのセルにも書かない(空配列)');
  } else {
    fail('空の値の分割が想定と違う');
  }
  if (joinChunks([]) === '' && joinChunks(['a', undefined, 'b']) === 'ab') {
    ok('joinChunks: 空セル(undefined)を挟んでも素直に連結する');
  } else {
    fail('連結の挙動が想定と違う');
  }
}
...
// ---------------------------------------------------------------------------
console.log('\n[6] exceedsSyncLimit / CAPACITY_WARN_PERCENT');

{
  const { CAPACITY_WARN_PERCENT } = await import(
    new URL('../docs/lib/sync.js', import.meta.url)
  );

  if (!exceedsSyncLimit('x'.repeat(SYNC_VALUE_LIMIT))) {
    ok(`ちょうど上限(${SYNC_VALUE_LIMIT.toLocaleString()}文字)は超過扱いにしない`);
  } else {
    fail('ちょうど上限で超過と判定されている');
  }
  if (exceedsSyncLimit('x'.repeat(SYNC_VALUE_LIMIT + 1))) {
    ok('上限を1文字でも超えたら超過と判定する');
  } else {
    fail('上限超過を検出できていない');
  }
  if (!exceedsSyncLimit('') && !exceedsSyncLimit(null)) {
    ok('空文字・null は超過扱いにしない');
  } else {
    fail('空の値が超過と判定されている');
  }
  // 1セルを超えても超過ではない(これが分割保存の狙い)
  if (!exceedsSyncLimit('x'.repeat(SHEET_CELL_LIMIT + 1))) {
    ok('1セルの上限(50,000)を超えても、分割保存できる範囲なら通す');
  } else {
    fail('1セル超えで誤って弾いている(分割保存が効いていない)');
  }
  ...
  const warnJson = 'x'.repeat(Math.ceil(SYNC_VALUE_LIMIT * (CAPACITY_WARN_PERCENT / 100)));
  if (capacityPercent(warnJson) >= CAPACITY_WARN_PERCENT && !exceedsSyncLimit(warnJson)) {
...

{
  ...
  if (!exceedsSyncLimit(worstCase) && capacityPercent(worstCase) < 70) {
