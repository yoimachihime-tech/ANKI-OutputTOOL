...
各タブの**まだ出力していないカード**を1回の操作で1つの`.apkg`にまとめる。
デッキ・ノートタイプは種別ごとに分かれたまま入るので、Anki側では今までどおり
別々のデッキに取り込まれる(`lib/apkg.js`の`buildApkg({groups})`)。

- 集める対象は「未出力」から「重複としてマークされているもの」を除いたもの。
  **重複クラスタは片方だけ残さず丸ごと対象外**にする(どちらを残すべきかは
  アプリには判断できないため)。件数は①の内訳に出る。
- DailyConversationは候補の実体がスプレッドシートなので、**このタブからは
  シートを読みに行かない**。DailyConversationタブの②で読み込み済みの行だけが
  対象になる(チェックボックスで含める/含めないを選べる)。
- Num・cards.dueの採番、出力済みの記録(`exported_at` / 打ち消し記録 /
  ローカルの出力済みID)の付き方は、各タブから個別に出力したときと同じ。
- シートの「Anki出力済み」列へのマークだけは行わない(必要なら
  DailyConversationタブから)。
- 回帰テスト: `tools/test_web_ui.mjs` の[19][20]。
